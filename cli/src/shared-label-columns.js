 ...
 * `fields` section leaves the boot bundle (VITE_SLICED_LABELS on). They are
 * therefore merged into every `core.<locale>.json` so they resolve everywhere.
 *
 * Derivation (re-run when adding/removing a literal label reference in shared code):
 *   grep -rhoE "\bt\(['\"][A-Z][A-Za-z0-9_]*['\"]" \
 *     tools/app-shell/src packages/app-shell-core/src \
 ...
 * silently regresses that label to the contract's raw English text when the flag
 * is on, so prefer keeping any literal reference here.
 */
