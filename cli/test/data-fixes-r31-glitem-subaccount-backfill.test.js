...
describe('R31 data-fix — column mapping mirrors GlItemProvisioningSupport 1:1', () => {
  ...
  it('composed name is "<name, truncated to fit> <searchKey/value>", falling back to the (possibly truncated) bare name when the code is blank', () => {
    assert.match(
      normApply,
      /CASE WHEN itc\.subaccount_code IS NULL OR itc\.subaccount_code = '' THEN left\(itc\.subaccount_name, 60\) ELSE left\(itc\.subaccount_name, GREATEST\(60 - length\(itc\.subaccount_code\) - 1, 0\)\) \|\| ' ' \|\| itc\.subaccount_code END/i,
    );
  });
...
describe('R31 data-fix — N2 truncation (C_Glitem.Name is varchar(60)) — FIXED, matches composeGlItemName/truncateToFit byte-for-byte', () => {
  ...
  it('created_items truncates the NAME portion only via left(), mirroring Java\'s truncateToFit(name, GL_ITEM_NAME_MAX_LENGTH - suffix.length()) — never truncates or drops the code', () => {
    ...
    assert.match(normApply, /left\(itc\.subaccount_name, GREATEST\(60 - length\(itc\.subaccount_code\) - 1, 0\)\) \|\| ' ' \|\| itc\.subaccount_code/i);
  ...
  it('@report lists subaccounts whose linked GL Item name is EXACTLY the deterministic truncation formula — precise, not a guess', () => {
    ...
    assert.match(
      normReport,
      /CASE WHEN ev\.value IS NULL OR ev\.value = '' THEN left\(ev\.name, 60\) ELSE left\(ev\.name, GREATEST\(60 - length\(ev\.value\) - 1, 0\)\) \|\| ' ' \|\| ev\.value END AS expected_truncated_name/i,
    );
