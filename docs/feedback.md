...
## [2026-08-26] ETP-5019 — Owner/admin role composition self-service silently overwrote the Admin role

**Component:** `UserRoleCompositionService#enforceOwnerProtection` (`com.etendoerp.go`); `AssignTemplateRolesControl.jsx` (`tools/app-shell/src/windows/custom/user/`)

**Symptom:** A tenant's owner/admin — or any user manually granted the classic client-admin "Admin" role — could open the `User` window's role-composition control and add template roles (Finance/Sales/etc.) to their own account. Doing so silently replaced their `Default_Ad_Role_ID` with a brand-new, empty personal composition role, losing the Admin role as a side effect. No error, no confirmation — just a demoted admin on next login.

**Root cause:** ETP-4830's original `enforceOwnerProtection` only rejected a DIFFERENT caller reassigning the owner's roles; the owner recomposing their OWN roles (`callerUserId.equals(user.getId())`) was treated as a no-op, on the theory that self-service was the safe case. That theory missed that `isReusablePersonalRole` explicitly excludes the client-admin role from reuse — so composing even one template role for an Admin-role holder, self-service or not, always minted a fresh personal role and overwrote `Default_Ad_Role_ID`.

**Fix:** the backend guard now rejects composition unconditionally whenever the target is flagged as the ETP-4830 owner (`OwnerSupport.isOwner`) **or** currently holds the client-admin role (`getDefaultRole().isClientAdmin()`) — whichever signal fires, regardless of who is asking. The frontend control detects the same condition and renders a locked message instead of the interactive editor, so the doomed interaction is never even offered. Full mechanism: `com.etendoerp.go`'s `docs/neo-headless.md` §8d; window-level detail: `docs/generated-custom-windows/user.md` → "Owner/admin composition lock (ETP-5019)".

**Lesson:** "only a different caller is risky" is not a safe default for a self-overwrite bug — the affected user acting on themselves can be exactly as destructive as a third party acting on them, especially when the underlying resource (a role) can't be recovered by re-selecting it (the Admin role is not offered back through the composition UI at all). When a guard exists to prevent State X from being silently destroyed, check whether self-service reaches State X too, don't assume the actor's identity is what makes it safe.

**Known non-blocking follow-ups (QA, not yet separately ticketed):**
- The frontend lock's `isAdminRoleHolder` detection depends on `fetchTemplateRoles()`/`fetchRolesOverview()` resolving successfully; on a network failure the `.catch()` resets `adminRoleId` to `null`, so the control fails OPEN — an admin/owner would briefly see the normal interactive editor instead of the locked message. Not a data-integrity risk (the backend rejection is unconditional and does not depend on this fetch), just a UX rough edge: a save attempt in that state would still fail with the backend's rejection message via error toast.
- ETP-4877 (existing-tenant owner-flag + personal-role retrofit backfill, still **Defined**, not yet written) must treat the owner as excluded from the personal-role-minting pass — see `docs/etendo-ad/onboarding-gaps.md` → L1 for the current cross-ticket coordination note.

---

## [2026-08-27] ETP-5019 (Bug A) — Owner's Email field always rendered blank

**Component:** `EtendoGoJwtServlet#resolveOrCreateClient` / `EtendoGoJwtSupport#applyClientAdminEmail` (`com.etendoerp.go`)

**Symptom:** Every tenant owner's "Correo electrónico" field in the `User` window rendered empty, even though the owner obviously has a real email (it's what they registered with). Confirmed via direct query: 69/69 pre-existing `EM_ETGO_Is_Owner='Y'` rows had `Email IS NULL`.

**Root cause:** Core Etendo's `InitialSetupUtility#insertUser` — the primitive `InitialClientSetup` calls to provision a brand-new client's admin `AD_User` — sets `Name`/`Description`/`Username` but never `Email`. Nothing downstream ever backfilled it, so the column was genuinely `NULL` in the DB, not a frontend display bug. `AD_User.Username` cannot be substituted directly either: onboarding names the FIRST environment a founder creates after their plain account email, but every LATER environment gets `<accountEmail>+<clientName>` (`EtendoGoJwtSupport#buildClientUsername`), to dodge the uniqueness constraint — so a naive "copy Username into Email" would corrupt the email for any owner founding a second tenant.

**Fix:** `EtendoGoJwtSupport#applyClientAdminEmail(username, email)`, called once from `EtendoGoJwtServlet#resolveOrCreateClient` right after the existing `applyClientAdminDisplayName` call — i.e. only in the NEW-client-creation branch, never on a subsequent login of an existing client (the resumable-existing-client branch, `validateExistingClient`, never calls it). Sources the value from `accountEmail` (the founder's verified `ETGO_Account` email held on the request), never from `clientUser`/`username`. A corrective data-fix, `R28-owner-email-backfill.sql`, resolves already-provisioned owners by mirroring the exact same exact-then-suffix-split algorithm `GoAccountResolver#findAccountByUsername` already uses at login time (never a new ad-hoc heuristic) — 69/69 owners resolved on the shared dev DB, run for real, idempotent on re-run. Full writeup: `docs/etendo-ad/onboarding-gaps.md` → "L2 — `AD_User.Email` NULL for pre-existing tenant owners".

**Lesson:** When a column silently stays `NULL` after onboarding, check what the *primitive* setup call actually writes (`InitialSetupUtility#insertUser` here) rather than assuming a higher-level wrapper fills every field it plausibly could. And when a corrective backfill needs "the real identity behind an `AD_User`", grep for an existing runtime resolver (`findAccountBy*`/`resolveAccount*`) before writing new join/matching logic — mirroring the live path exactly avoids the corrective SQL ever disagreeing with what login itself would resolve.

---

## [2026-08-27] ETP-5019 (Bug B) — Business-Partner-contact users leaking into the Users grid

**Component:** `UserRoleAssignmentHandler#excludeContactOnlyUsers` (`com.etendoerp.go`)

**Symptom:** The `User` window's list grid showed rows like "Default Customer Contact" for every client — `AD_User` records that exist only to carry contact info for a Business Partner Contact (auto-created from the BP window's "Contacts" tab), never meant to log in or hold a role.

**Root cause:** The Users list `GET` had no filter distinguishing a real, login-capable user from a BP-contact placeholder row. Role count looked like an obvious filter but was ruled out empirically: sampling showed 42 genuine, not-yet-assigned real users with zero roles — filtering on role count would have hidden them too. `username` blankness turned out to be the correct structural signal: 110/110 sampled BP-contact rows had `username IS NULL`, while 185/185 sampled real users (including the 42 zero-role ones) had it set — because the same handler already derives `username` from `email` on every real create (see the invite-on-create flow).

**Fix:** `UserRoleAssignmentHandler#excludeContactOnlyUsers`, a pre-hook on the `user` list `GET` (fires only when no record id is present), injects a `_neoWhere` HQL predicate (`e.username is not null and e.username <> ''`) before the default CRUD list fetch runs. Filtering at the HQL level — not by post-filtering the fetched rows — was a deliberate choice: post-filtering after the DB-level `LIMIT`/`OFFSET` had already applied would corrupt pagination (a page could come back short even though more matching rows exist). Entirely backend-side; nothing in `decisions.json` or the generated `UserHeaderTable.jsx` filters changed. Full writeup: `docs/generated-custom-windows/user.md` → "Contact-only rows excluded from the list (ETP-5019)".
...
## [2026-08-31] ETP-5087 — TBAI offered on purchase invoices regardless of Basque territory

**Component:** `getInvoiceFiscalTargets` (`tools/app-shell/src/windows/custom/shared/fiscalTargets.js`)

**Symptom:** A purchase invoice offered TBAI/Batuz sending (`Enviar a SIF` button, SIF tab, SIF bottom-panel block, badges) under the `tbai` and `sii+tbai` fiscal profiles no matter which Basque territory the organisation's active TBAI configuration targeted. TicketBAI (Batuz) only applies to Bizkaia — Álava and Gipuzkoa use their own separate regimes — so purchase invoices under those two territories should never have been TBAI-eligible.

**Root cause:** `getInvoiceFiscalTargets(specName, profile)` gated TBAI purely on `isSales` (`specName === 'sales-invoice'`), treating every non-sales spec as ineligible for TBAI — a document-type-only check. It never looked at the org's actual TBAI territory, so any purchase invoice with a `tbai`/`sii+tbai` profile was silently treated as eligible even when the configured territory wasn't Bizkaia.

**Fix:** `getInvoiceFiscalTargets` now takes a third `territory` parameter; internally `showTbaiForDoc = isSales || (isPurchase && territory === 'BIZKAIA')` replaces the old `isSales`-only checks in the `tbai` and `sii+tbai` branches. `territory` is sourced from `tbaiRecord?.etsgSifTerritory` (via `useFiscalConfig`) and threaded through every consumer: `SendToSifButton.jsx`, `sifSending.js` (`getPendingSifTargets`), `useInvoicePreview.js`, `InvoicePreview.jsx`, `useFiscalStatus.js`, `useSifFieldPatcher.js`, and `PurchaseInvoiceHeaderTable.jsx`. It defaults to `null` everywhere, so a missing/unresolved TBAI config fails closed (TBAI stays hidden for purchase), never open. Sales invoices are unaffected — TBAI stays always-eligible for sales regardless of territory. Full writeup: `docs/generated-custom-windows/purchase-invoice.md` → "TBAI territory gating for purchase invoices (Batuz) — ETP-5087".

**Lesson:** A fiscal-system eligibility check driven only by document type (`isSales`/`isPurchase`) can silently miss a narrower real-world condition (which of several sub-regimes under the same "system" name actually applies). When a fiscal/tax system name covers multiple territories or sub-variants with different rules (TBAI here; SII already has `sii`/`sii-navarra` as a precedent), check whether the gating function needs the specific config record, not just the coarse profile string.

---

## [2026-09-01] ETP-5087 (review cycle 2) — Transient org-flash pattern in per-row async fiscal resolution

**Component:** `PurchaseInvoiceHeaderTable.jsx#resolveRowTargets` (`tools/app-shell/src/windows/custom/purchase-invoice/`)

**Symptom (caught in review, never shipped):** an earlier draft of the per-row TBAI/Batuz list column (see "List TBAI/Batuz column + per-row org resolution — ETP-5087" in `docs/generated-custom-windows/purchase-invoice.md`) fell back to the globally-selected org's fiscal profile/territory for any row whose own org hadn't resolved yet in the new `useOrgFiscalConfigs` batch cache — the natural first instinct, since the column-level visibility check already does exactly that while `data` is still empty. For a single row, though, that fallback briefly renders a REAL, believable `FiscalStatusBadge` sourced from the wrong org's config during the fetch window, not an obviously-loading placeholder — a genuine cross-org data bleed, just a transient one.

**Root cause:** two different call sites reasonably want two different answers to "what do I show while the async per-org fiscal config hasn't arrived yet". Page-level ("should this column exist at all") tolerates the global fallback because it only affects whether a column is rendered, never which org's data appears in a specific cell. Row-level ("what does THIS row's badge say") does not — the fallback data is real and specific to a *different* org, so showing it isn't a generic loading state, it's a wrong answer that happens to be indistinguishable from a right one until the real value overwrites it a moment later.

**Fix:** `resolveRowTargets` returns `{ showSii: false, showTbai: false, showVerifactu: false }` (i.e. "not yet eligible") for any row whose org has no entry in the `useOrgFiscalConfigs` cache yet, instead of falling back to the global `profile`/`territory`. The cell renders "—" until the real per-org config resolves, then flips to the correct badge — never a badge borrowed from the wrong org.

**Lesson — generalizable beyond this window:** whenever a UI merges (a) an async, per-row/per-entity resolved value with (b) a synchronous, globally-available fallback of the *same shape*, "fall back to the global value while loading" is only safe if the global value cannot plausibly disagree with the eventual per-row value in a way that looks like a valid answer. If the fallback and the real value are both instances of "a real config for *some* org" (fiscal profile, territory, permissions, pricing, currency, etc.), a mismatch renders as a silently wrong-but-plausible value during the loading window, not as a visible loading state — worse than showing nothing, because nothing looks broken and this looks correct. Default to a neutral/absent state ("—", disabled, hidden) instead of a same-shaped fallback whenever the two values could differ per-entity; reserve the "use the global value while loading" shortcut for genuinely page-scoped decisions (does this column exist at all) where a transient wrong answer costs nothing.
...
## [2026-09-02] ETP-5087 — A column read a field no backend handler populated for this window, and degraded to a plausible default instead of failing

**Component:** `PurchaseInvoiceHeaderTable.jsx` — the "Estado Batuz" list column (`tools/app-shell/src/windows/custom/purchase-invoice/`)

**Symptom:** every purchase invoice in the list showed "Pendiente" in the Batuz column — including invoices that had demonstrably been sent to Batuz. The column was never right for any row, yet nothing in the UI, the console or the network tab looked broken.

**Root cause — part 1, a real field with no producer on this window.** The cell rendered `row.tbaiSyncEstado ?? 'Pendiente'`. `tbaiSyncEstado` is a perfectly real field — it just is not a contract field, and nothing was populating it for purchase invoices. It is injected server-side per row by `TbaiSyncStatusInjector.inject()`, called from `SalesInvoiceHeaderHandler.afterHandle()` in `com.etendoerp.go`. The renderer was copied from `sales-invoice`'s `InvoiceHeaderTable.jsx`, where that injector runs; `PurchaseInvoiceHeaderHandler.afterHandle()` did not call it, so on a purchase row the property was always `undefined`. Grepping `contract.json` would NOT have found the field even in the window where it works — the producer is a Java handler, not the contract. The `?? 'Pendiente'` fallback — written to mean "not sent yet" — silently absorbed "nothing produces this here" as well, and the two are indistinguishable on screen. Meanwhile the invoice's own send flag `tbaiIssent` (`EM_Tbai_Issent`) sat in the contract at `visibility: "discarded"`, never serialised to the browser, so there was no second source to disagree with the lie either.

**Root cause — part 2, the over-engineering.** The same column was then built on an async, per-row org resolution (`useOrgFiscalConfigs` + `resolveRowTargets`) so each row could be gated by its OWN org's TBAI territory. The sibling SII column in the same file had always resolved this synchronously from the top-nav org and had never misbehaved. The added asynchrony produced three consecutive shipped regressions: (1) the Batuz column never appeared on first render; (2) fixing that broke the **SII** column, because an intermediate version folded both into the same per-row reduction; (3) the hook returned the same mutated `Map` reference every render, so callers memoizing off it never recomputed and the column stayed frozen until an unrelated re-render shook it loose.

**Fix (two repos, one ticket):** in `com.etendoerp.go`, wire `TbaiSyncStatusInjector.inject(dataArr)` into `PurchaseInvoiceHeaderHandler.afterHandle()` so purchase rows carry the same real, multi-state `tbaiSyncEstado` (`Recibido` / `Rechazado` / `Error`) sales rows always did — the missing producer, not the field, was the defect. In the frontend, keep `tbaiSyncEstado` as the PRIMARY source and fall back to the invoice's own send flag only when it is absent: `row.tbaiSyncEstado ?? (isSent(row.tbaiIssent) ? 'Enviada' : 'Pendiente')`. That required promoting `tbaiIssent` to `{ "visibility": "readOnly", "form": false }` in `artifacts/purchase-invoice/decisions.json` so it reaches `frontendContract`, plus a new `Enviada` badge state — deliberately not `Recibido`, which asserts acceptance by the Diputación that a mere send flag cannot back. `isSent` (`value === true || value === 'Y'`) is exported from `shared/sifSending.js` because NEO may serialise an AD boolean as the character `'N'`, truthy in JS. Separately: delete `useOrgFiscalConfigs.js` and its test, and resolve both fiscal columns from one synchronous `getInvoiceFiscalTargets(specName, profile, territory)` memo, symmetric with SII. Sales-invoice was not touched.

**Why the fallback stays even after the injector ships:** the two sources answer different questions. `tbaiSyncEstado` is the outcome of the submission and is the only one that can say *rejected*; `tbaiIssent` only says the invoice was submitted. Reading the flag first would let a rejection render as a cheerful "Enviada" — so the real state must win, and the flag is kept purely to cover an invoice with no row in `tbai_syncinvoice` yet, or a backend that predates the injector wiring. The column then never defaults to a fabricated status and never hides a rejection.
