...
1. New build-time slicer `cli/src/slice-labels.js` (run as a build prebuild, **decoupled from `make regen`**) that emits a per-window, label-only, bilingual slice (gitignored — see §4 delivery model).
...
**The `.manifest.json` is the frontend generator's concern (F2) — this design does not touch it.** `generated/.manifest.json` is defined as `{ contractChecksum, … }` for F2, but `generate-frontend.js` does not emit it for real artifacts yet (only the 3 validator fixtures have one), so F2 stays in `skipped(...)`. The slicer writes **no** manifest and **no** checksum: slices are gitignored build artifacts (see §4 delivery model), and F18 validates them by reproducing-and-comparing content, so there is nothing to checksum and no dependency on the F2 manifest emitter.
...
> **Delivery model — DECISION (2026-06-29, supersedes earlier "committed slices"):** the per-window `labels.js` and the shared `core.*` are **build-time artifacts: gitignored, never committed, regenerated on every build** (Option B). They are **NOT** part of `make regen` (the slicer is decoupled from the artifact pipeline) and there is **no `.manifest.json` checksum** — since the slices are regenerated from current source each build, they cannot go stale, so F18 validates by reproducing-and-comparing content (not a stored checksum). Rationale: keeps the repo free of ~60 autogenerated files, sidesteps pre-existing F16/F3 debt in demo windows, and matches the offline-regen-check's "clean tree after regen" invariant. The build-time prebuild wiring lands with Phase 2B (when the runtime consumes the slices). The sections below describe the slicer mechanics; read them through this delivery model.

A new generator `cli/src/slice-labels.js`. The source of truth `extract-labels.js` is **unchanged** — the slicer is a pure transform over already-extracted JSON, **no DB access**, so it is safe in offline CI and as a build prebuild.
...
3. Emit `artifacts/<win>/generated/web/<win>/labels.js` (gitignored):
...
The slicer writes **only** `labels.js` (no manifest, no checksum). A `--check` mode recomputes every window's slice in memory and compares against the on-disk `labels.js` — useful as a local sanity check; it is not a commit gate, since slices are not committed.
...
> **Build-time integration (Phase 2B):** the slicer runs as a **build prebuild** (before Vite), emitting all windows' `labels.js` + `core.*` into gitignored paths. It is decoupled from `make regen` so a regen leaves the tree clean (the offline-regen-check passes). Since both inputs (locale JSONs, contracts) are committed and the transform is DB-free, the prebuild is fast and offline-safe. `core.<locale>.json` is global (one per locale), emitted once per prebuild run.
...
```
extract-labels.js (DB, unchanged) ──▶ locales/en_US.json + es_ES.json  (full, source of truth, committed)
                                              │
                 build prebuild ──▶ slice-labels.js  (offline transform, no DB; NOT in make regen)
                                              ├─▶ artifacts/<win>/generated/web/<win>/labels.js   {en_US,es_ES} label-only  [gitignored]
                                              └─▶ packages/app-shell-core/src/locales/generated/core.<locale>.json        [gitignored]
...
| File | Change |
|---|---|
| `cli/src/slice-labels.js` | **new** — per-window slicer + `core.*` emitter + `--check`. Writes only `labels.js` / `core.*` (no manifest, no checksum). **Decoupled from `make regen`.** |
| `.gitignore` | ignore `artifacts/*/generated/web/*/labels.js` + `packages/app-shell-core/src/locales/generated/` (build-time artifacts, never committed) |
| build prebuild (Phase 2B; e.g. `tools/app-shell` `prebuild`) | run `slice-labels.js --all` before Vite so slices exist at build time |
| shared label set (Phase 2B, location TBD) | columns used by cross-window shared components, merged into `core.*` |
...
| `packages/app-shell-core/src/locales/generated/core.<locale>.json` | generated build output, per locale (gitignored) |
| `artifacts/<win>/generated/web/<win>/labels.js` | generated build output, per window (gitignored) |
