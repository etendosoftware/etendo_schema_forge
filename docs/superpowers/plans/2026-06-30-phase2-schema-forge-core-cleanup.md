...
## Addendum 2 (discovered while actually pushing): Makefile test targets, and a broader Makefile follow-up

Pushing the branch (`git push`) triggers the pre-push hook, which runs `make test-all-coverage` for real — this caught what no static grep or in-repo `npm test` run could: `test`, `test-all-coverage`, `test-ci`, `test-ci-coverage` all still referenced `tools/app-shell/` and `artifacts/` (deleted in Task 3). Fixed all four to cover only what remains (`cli/test`, `packages/schema-forge-core/test`, `packages/app-shell-core`), and had to add an explicit `--coverage.reporter=lcov` flag since vitest's default coverage reporters don't produce the `lcov.info` file the Makefile's merge step expects.

**Not fixed, deliberately** — the rest of the ~500-line `Makefile` (`dev`, `dev-mock`, `build`, `deploy`, `regen`/`regen-check`/`dump-delta`, `switch-to-es`/`switch-to-ar`, `clean`, and `validate-pipeline`/`method-budget`/`window-leak-budget`) still references `tools/app-shell`/`artifacts` throughout. All of these only make sense run from a repo that has real window content — they either build/deploy the actual app, or validate/ratchet against real generated output. This repo has neither. A full pass to strip or clearly gate these targets (most should probably be deleted from `schema_forge_core`'s `Makefile` entirely, not adapted — there is no app here to `make dev`, no windows here to `make regen`) is real, separate follow-up work, tracked here rather than attempted under the pressure of unblocking a push.
