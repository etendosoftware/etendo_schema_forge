...
3. **[Superado en ETP-5045] El estado de pago y la deduplicación de webhooks ya NO viven en memoria del proceso.** `CheckoutPaymentRegistry` (dos `ConcurrentHashMap` estáticos que se perdían en cada reinicio) fue eliminado. Hoy la correlación "este `requestId` ya fue pagado" es una fila de `ETGO_CHECKOUT_REQUEST` (`CheckoutRequestStore`) y cada evento de Stripe es una fila de `ETGO_BILLING_EVENT` (`BillingEventStore`, una por `event_id` con constraint único). Un reinicio del backend entre el webhook y el polling ya no pierde el "pagado", y un reintento de Stripe tras el reinicio se cuenta como duplicado en vez de reprocesarse. Ambas tablas se consultan sin acceso a DB desde ventanas Classic de solo lectura (**Checkout Request**, con pestaña hija **Billing Event**, y **Billing Event**). Ver §Checkout en `03-reglas-estados-y-validaciones.md` y CAP-CHK-04.
