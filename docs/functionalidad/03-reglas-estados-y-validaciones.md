...
- **[Hecho] El estado de pago es durable (ETP-5045).** El "pagado" es una fila de `ETGO_CHECKOUT_REQUEST` que el webhook avanza a `PAID` (`CheckoutRequestStore.java:146-174` `recordPaid`), y el polling la lee (`CheckoutRequestStore.java:188-206` `find`, `:219-228` `isPaidFor`). Un reinicio del backend entre el webhook y el polling ya no pierde nada. El antiguo `CheckoutPaymentRegistry` (`ConcurrentHashMap` en memoria del proceso) fue eliminado.
- **[Hecho] Dedup de eventos de webhook durable, una fila por `event_id`** en `ETGO_BILLING_EVENT` (`BillingEventStore.java:130-152` `claim`). El gate de idempotencia es el constraint único `ETGO_BILLEVT_EVENT_UQ`, no un mapa: la primera entrega inserta la fila en `RECEIVED` (`:214-244` `insertReceived`); una reentrega choca contra el constraint y solo incrementa `DUPLICATE_COUNT` + `LAST_DUPLICATE_AT` (`:279-296` `recordDuplicate`), respondiendo `200 {"received":true}` sin reprocesar — también tras un reinicio o en otro nodo. Si esa actualización no toca ninguna fila, la premisa era falsa (el insert falló por otro constraint) y se relanza la excepción original → 500, para que Stripe reentregue en vez de dar el evento por recibido.
- **[Hecho] Ciclo de vida del evento: `RECEIVED → APPLIED | IGNORED | FAILED`, con candados distintos.** `APPLIED` (el pago quedó registrado) es terminal **y está garantizado en código**: toda escritura de otro resultado lleva la guarda `eventResult <> 'APPLIED'` (`BillingEventStore.java:311-354` `updateResult`), así que una falla tardía en una reentrega no puede reabrir un evento ya aplicado (spec `testAppliedIsTerminalAgainstALaterFailureOrIgnore`). `IGNORED` es terminal por intención pero **no está bloqueado**: un `markFailed` posterior sí lo sobrescribe y vuelve la fila re-reclamable — deliberado, porque una entrega posterior puede traer la correlación que a la ignorada le faltaba (spec `testIgnoredIsNotLockedTheWayAppliedIs`). `FAILED` no es terminal: la siguiente entrega del mismo `event_id` vuelve la fila a `RECEIVED` atómicamente y se procesa como nueva (`:247-264` `reclaimFailed`), de modo que el propio reintento de Stripe repara una falla transitoria. `PROCESSED_AT` es first-write-wins.
- **[Hecho] Falla del handler → HTTP 500 `CHECKOUT_WEBHOOK_FAILED`** y la fila queda `FAILED` (`EtendoGoJwtServlet.java:474-519` `handleCheckoutWebhook`; `BillingEventStore.java:182-188` `markFailed`, que nunca lanza). El 500 es deliberado: hace que Stripe reintente. En `FAILURE_REASON` se guarda una frase fija más el **nombre de la clase de la excepción**, nunca su mensaje: un mensaje controlado por el proveedor puede citar fragmentos del payload, y esa columna debe quedar operacionalmente segura. El detalle va al log.
- **[Hecho] Correlación con el checkout:** la fila del evento guarda siempre el `metadata.request_id` crudo (`REQUEST_ID`) y, solo si esa solicitud existe en `ETGO_CHECKOUT_REQUEST`, la FK `ETGO_CHECKOUT_REQUEST_ID` resuelta en el momento del claim (`BillingEventStore.java:214-244`; `CheckoutRequestStore.java:384-395` `findByRequestId`). Un evento con un `request_id` desconocido queda **`IGNORED`** con motivo `unknown checkout request` (`recordPaid` devuelve `false` y el servlet lo marca así, `EtendoGoJwtServlet.java:547-552`) y un error en el log — no marca nada como pagado. Por lo tanto **`APPLIED` siempre significa que hubo un pago registrado**, nunca solamente que el handler terminó.
- **[Hecho] `FAILURE_REASON` no es solo de fallas.** La misma columna guarda el motivo de `IGNORED`, así que en una instancia sana la mayoría de las filas dicen `unhandled event type` en una columna llamada "failure reason" — es por diseño. Los tres motivos de ignorado son `unhandled event type`, `missing correlation metadata` y `unknown checkout request`.
- **[Hecho] Nunca se persiste el payload crudo ni datos de tarjeta.** `PAYLOAD_SUMMARY` es una allow-list (`data.object.{id,customer,subscription,livemode,payment_status,amount_total,currency,mode}` + `metadata.request_id`, máx. 2000 caracteres). La política vive en `WebhookPayloadSummary.java:54-83` `summarize` — JSON puro, sin DAL, para que el seam genérico `EventStore` arme el resumen sin depender del store concreto; `BillingEventStore.java:201-203` es solo un delegado.
- **[Hecho] Auditoría sin DB:** ventanas Classic de solo lectura **Checkout Request** (con pestaña hija **Billing Event**, enlazada por la FK) y **Billing Event** (todas las filas, con o sin FK), visibles como System Administrator. Al leer la ventana, tener presente lo de `FAILURE_REASON` arriba: el campo también explica los `IGNORED`, no solo los `FAILED`.
