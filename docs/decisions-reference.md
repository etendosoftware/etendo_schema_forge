...
| `disableWhenEmpty` | boolean | `false` (key omitted) | Disables the Confirm button while the entity has no child lines (`!hook.childrenLoading && hook.children.length === 0`). Emitted only when `true`. Used by `purchase-order`, `sales-order`, `sales-quotation`, the invoice windows, and (ETP-5408) `return-material-receipt` / `return-to-vendor-shipment`. |
