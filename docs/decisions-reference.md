...
| `displayLogicJs` | string \| undefined | `undefined` | Raw JavaScript expression (not AD-logic syntax) for conditional visibility, evaluated against the record with `record` bound in scope — e.g. `"!record.id"` to hide a field only on the CREATE form (shown again once persisted). Compiles into the same generated `displayLogic: (record) => …` function property `EntityForm.jsx`'s `evalDisplayLogic` reads to filter `displayFields` — a field failing this check is removed from the rendered form entirely, not merely disabled. Real examples: `assets`/`asset-group`'s several fields gated on `"record.depreciate === true \|\| record.depreciate === 'Y'"`, and `user`'s `password` (ETP-4830, `"!record.id"` — hidden on the create form now that admin-typed passwords no longer bypass the invite-email flow, shown again once the user record is saved so an existing user's password can still be reset). |
