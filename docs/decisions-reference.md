...
`InlineLinesPanel` renders this as a per-row "Dimensiones contables" badge summary (up to 2 filled dimensions + "+N" overflow, or a dashed "+ Add dimensions" trigger when empty) that expands a full-width sub-row of selectors on click — see `docs/ui-customization.md` §14b for the full UX and the shared `DimensionsPanel.jsx` building blocks.
