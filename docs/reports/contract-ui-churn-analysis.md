...
| L4 ✅ | ~~`DataTable.jsx:28,39`~~ → `components/contract-ui/lookupDrawers.js` | ~~`import InternalConsumptionProductSearchDrawer` + `LOOKUP_DRAWERS['internal-consumption-product']`~~ | ✅ Resuelta | **Ya no existe.** ETP-4525 (`122a3a5d5`) unificó los drawers per-window en `ProductStockSearchDrawer` (genérico) y sacó el registro a su propio módulo. `DataTable.jsx` no importa ningún componente de una sola ventana ni contiene literal de ventana alguno — el scanner reporta 0 sobre ese fichero. Queda solo la deuda de *lazy loading* (T05), diferida. |
...
5. **Mover algo a un módulo que los tests mockean ES un cambio visible para los tests, aunque no cambie el comportamiento.** Descubierto en ETP-4708 (T16): mover un helper de `DetailView.jsx` al módulo `@/hooks/useLineGrossAmount` rompió 6 tests, porque `DetailView.lineCalloutFlow.vitest.jsx` y `DetailView.currencyConversionBehavior.vitest.jsx` reemplazan ese módulo entero con una **factory mock** que sólo provee `useLineGrossAmount` — cualquier import nombrado nuevo desde ahí resuelve a `undefined`. Como R1 prohíbe tocar tests, hubo que quedarse con la opción de peor cohesión. **Esto afecta a toda la Ola 3**, que no es otra cosa que sacar piezas de este fichero: antes de elegir el destino de una extracción, comprobar si algún test mockea ese módulo (`grep -rn "vi.mock('<destino>'" __tests__/`). Un destino nuevo y propio (como `TabStrip.jsx`) no tiene ese problema; uno ya mockeado sí.
6. **Sacar una fuga a otro fichero no es pagarla.** El ratchet `sf-window-leak-budget` sólo escanea las rutas de `cli/window-leak-budget.json`: al mover `TAB_ICONS` a `TabStrip.jsx` el conteo cayó de 4 a **0** con las cuatro fugas intactas, y un `--update` en ese momento habría fijado un baseline que no vigila nada. Toda extracción que se lleve literales debe **añadir su fichero destino a `paths` en el mismo commit**.
...
> **Estado de ejecución (2026-07-29, ETP-4708).** **T01 cerrado.** El "queda replicarlo" de la nota anterior está hecho, pero **no vía ESLint**: se replicó el mismo ratchet sobre las líneas de los JSX con el bin `sf-file-lines-budget` (core `cli/src/file-lines-budget.js`, 14 tests) + baseline `cli/file-lines-budget.json` + job `file-lines-budget` en `.github/workflows/ratchet-guards.yml` + `make file-lines-budget`. Los baselines de partida (**DetailView 5.156**, **DataTable 2.058**) muestran cuánto creció el problema desde que se midió este informe (§9.1 media 3.914 en DetailView): **+1.242 líneas sin guardarraíl**. El job queda inerte hasta que el pin de core publique el bin.
...
|----|----|----|----|----|----|----|----|
| [x] | T01 | Añadir regla ESLint `max-lines`/`complexity` a `DetailView.jsx` y `DataTable.jsx` que **falle el CI si crecen** (S1). *Hecho como **ratchet** en vez de umbral ESLint fijo: bin `sf-file-lines-budget` (`cli/src/file-lines-budget.js` en core, 14 tests), baseline `cli/file-lines-budget.json` (DetailView **5.156**, DataTable **2.058**), job `file-lines-budget` en `.github/workflows/ratchet-guards.yml` y `make file-lines-budget`. Un umbral fijo o bloquea todos los PR hoy o se pone tan alto que permite justo el crecimiento que debería frenar; el ratchet falla solo si el fichero **crece** y `--update` baja el baseline al extraer sub-componentes (objetivo §9.2: ningún fichero > ~600 líneas). **Entra en vigor cuando el pin de core publique el bin.*** | XS | 🔴 | CI | **V:** ✅ 14 tests del ratchet (crece⇒exit 1, baja⇒`--update`, `--update` no sube un baseline crecido, fichero ausente⇒exit 2) · **E:** n/a | — |
...
| ✅ | # | Tarea atómica | Compl. | Impacto | Pipeline | Step de test (V / E) | Dep. |
|----|----|----|----|----|----|----|----|
| [x] | T04 | **L4** — Mover registro `LOOKUP_DRAWERS` fuera de `DataTable.jsx`. *Ya satisfecho por **ETP-4525** (`122a3a5d5`), antes de esta rama: el registro vive en `components/contract-ui/lookupDrawers.js` con su helper `resolveLookupDrawer()`, y el scanner de fugas reporta **0 literales de ventana en `DataTable.jsx`** (las 8 del baseline están todas en `DetailView.jsx`). La ruta `lib/lookupDrawers.js` del enunciado queda **superada**: `src/lib/` es hoy 100 % libre de componentes, y mover allí un registro de componentes JSX invertiría el layering (`lib → components`) además de romper 2 ficheros de test por aserciones de ruta (R1). Intención cumplida; la letra del enunciado, no.* | XS | 🟠 | app-shell | **V:** ✅ `lookupDrawers.vitest.jsx` (6 tests) · **E:** n/a (sin cambio de código) | — |
| [ ] | T05 | **L4** — Convertir el import del drawer a `React.lazy`. ***Diferido a ticket propio.** Verificado empíricamente (2026-07-29): `lazy()` devuelve `{$$typeof: Symbol(react.lazy), …}`, nunca el export del módulo, así que **fallan los 6/6 asserts de identidad** de `lookupDrawers.vitest.jsx`; y `InlineLinesPanel.vitest.jsx > renders readonly LookupTrigger` falla sin boundary `Suspense`, porque los 3 puntos de montaje renderizan `<Drawer open={open}>` incondicionalmente (el drawer devuelve `null` cerrado). El único camino es editar tests ⇒ **conflicto directo con R1**. La premisa también quedó obsoleta tras ETP-4525: el registro ya no apunta a ningún componente de una sola ventana — ambas entradas son drawers genéricos compartidos — y el montaje incondicional dispararía el `import()` en el render de la tabla, sin diferir nada. Rehacer como ticket cuyo alcance **declarado** sea reorientar esos asserts de identidad a comportamiento y añadir los `Suspense` (superficie de regresión por flash de fallback bajo todas las ventanas ⇒ aplica la regla blast-radius).* | S | 🟠 | app-shell | **V:** pendiente en su propio ticket · **E:** `internal-consumption-line-entry` (abre drawer) | T04 |
...
| [x] | T11 | **L1** — Emitir `tab.icon` en el generador **solo si ≠ default** (`wrapIf`). *Hecho en core (`90a126a93`): `buildTabIconPart()` aplicado a los **tres** puntos de emisión — `customPanelTabs[].icon` (pricing), `extraTabs[].icon` (**sif**, el caso que motiva la tarea) y `attachments.icon` (tab propiedad del generador). Los tres, o SIF se queda sin icono. 9 tests, incluido control de igualdad byte a byte.* | S | 🟡 | gen | **V:** ✅ `cli/test/tab-icon.test.js` — emite `icon` solo cuando se declara; nada si ausente · **E:** n/a | T10 |
| [ ] | T12 | **L1** — Sustituir `TAB_ICONS['custom:sif']` por un resolutor de iconos alimentado por `tab.icon` (L11). ⚠️ **Corrección al enunciado (aprobada 2026-07-29):** este informe decía *"mapa **lucide** resuelto desde `tab.icon`"*. **No debe ser lucide ni ninguna otra librería: `tab.icon` lleva un nombre LÓGICO** (`'shield'`, `'attachment'`, `'pricing'`, `'warehouse-products'`). Motivo: `SideMenu.jsx` importa de `lucide-react` **y** de `@phosphor-icons/react`, y su `ICON_MAP` es un shim de compatibilidad lucide→Phosphor — la app está migrando fuera de lucide. Como `decisions.json` es un contrato que se regenera en todas las ventanas, grabar ahí nombres de librería convertiría el final de esa migración en una reescritura masiva + regen completa; con nombres lógicos queda en **un solo fichero resolutor**. Es el mismo principio de "declarar intención, no implementación" que el informe aplica en el resto: se sigue su lógica en contra de su letra. **Ojo, no sería un segundo registro sino el cuarto**: ya existen `TAB_ICONS` (4 entradas), `SideMenu.ICON_MAP` (20) y `CommandPalette.ICON_MAP` (8), ninguno exportado. El resolutor único va en `components/ui/iconRegistry.jsx` (**no** en `lib/`: `lib/` es 100 % libre de componentes). **Partir en dos:** Fase A = `resolveIcon` + `TAB_ICONS`; Fase B = SideMenu/CommandPalette en PR propio — una tarea de iconos de tab no puede reescribir la navegación global (regla blast-radius). El fallback `List` debe preservarse exactamente. **El resolutor debe servir a los dos caminos:** las ventanas de pipeline reciben `icon` emitido por el generador, y las **hechas a mano** (p. ej. `windows/custom/warehouse/index.jsx:41`, origen de `TAB_ICONS['products']`) construyen sus descriptores en JSX sin pasar por el generador y declaran `icon:` en línea. Ambas convergen en el mismo contrato `tab.icon`, así que ninguna necesita soporte especial — pero el resolutor no puede asumir que todo descriptor viene del generador. | XS | 🟡 | app-shell | **V:** resuelve icono por nombre lógico + fallback `List` · **E:** spec SIF (icono visible) | T11 |
| [ ] | T13 | **L1** — Regenerar y validar R2. *Corrección de alcance: no son 2 ventanas sino **4-5** — `sales-invoice` y `purchase-invoice` (ambas declaran el tab `sif` vía `extraTabs`), `product` (`pricing` vía `customPanelTabs`) y las ventanas con `attachments`. `TAB_ICONS['products']` **no** entra: viene de una ventana hecha a mano (`windows/custom/warehouse/index.jsx:41`) que construye sus descriptores en JSX sin pasar por el generador, así que declara `icon:` en línea.* | XS | 🟠 | test | **V:** n/a · **E:** spec SIF + `attachments` tras regen | T12 |
