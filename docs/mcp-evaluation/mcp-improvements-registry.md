...
| **IMP-46** | The SPA renders a vector-search failure as *no results* | P2 | C9 | 0 / 2 | ♻️ | `schema_forge` | ⏳ open | Registered 2026-09-15, found while aligning the vector target names. `useVectorSearch.js` maps **any** non-`ok` response to an empty match list (`if (!response.ok) throw` → `catch` → `setMatches([])`), so a `422 unknown_vector_target`, a `503` with DB Extended absent and a genuine zero-match query are the same thing on screen: *nothing found*. This is **the UI-side twin of IMP-41**, which was resolved on the MCP surface precisely because *absent* and *forbidden* being indistinguishable is a defect — the same conflation survives untouched in the SPA. It is also what makes the two-sided target declaration (IMP-47) dangerous rather than merely untidy: a one-sided rename is invisible to the user. `Done when:` a failed vector search is distinguishable from an empty one in the UI. Not measured: whether other SPA fetches share the pattern | `tools/app-shell/src/hooks/useVectorSearch.js` |
| **IMP-47** | Nothing enforces that a window's vector target equals its spec name and exists server-side | P3 | C9 | 0 / 1 | ⚙️ | `schema_forge_core` | ⏳ open | Registered 2026-09-15. The same key is declared twice and validated in neither place: `ETARC_VECTOR_SEARCH_TARGET.SEARCH_KEY` is what the server and the MCP answer with, `artifacts/<spec>/decisions.json → window.vectorSearch.target` is what the SPA sends — and neither side can see the other, because the contract is bundled into the app and `ETGO_SF_*` carries **no vector column at all** (verified: zero columns matching `vector` across `etgo_sf%`). ETP-5335 made the two equal and made the key the spec name, which is what lets a match be read (`neo_get(spec:<target>, entity:<primaryEntity>, id:<match.id>)`) — but that is a convention held by agreement, and one-sided drift surfaces as IMP-46's silent *no results*. A validator rule (F11) comparing contract targets against the `ETARC` sourcedata would settle it without DB access. `Done when:` a one-sided rename fails at commit time. Cost noted: the validator lives in `schema_forge_core`, so this needs a publish and a version bump, not a local edit | `docs/decisions-reference.md` · `neo-headless.md` §4.9a |
