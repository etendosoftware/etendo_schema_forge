...
| **IMP-18** | Report unknown names in a `fields` projection | P2 | C3 | 0 / 3 | ⚙️ additive | `com.etendoerp.go` | ⏳ open | B8, **C15** · `salePrice`/`purchasePrice`/`stock` dropped in silence, no `warnings` array · **re-confirmed unchanged 2026-08-10, and now sharper than when it was registered**: IMP-12 shipped `unknownFields` on `neo_schema`'s `fields` argument, so the same argument name on `neo_list` has two behaviours — this is a consistency defect with the fix pattern sitting one tool over, not a missing feature. It is also the **sole** reason frozen task 3 still fails, i.e. the cheapest item on the board relative to its effect: closing it alone takes M2 from 80 % to 100 % | audit §5 |
