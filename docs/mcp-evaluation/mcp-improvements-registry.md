...
| **IMP-41** | `neo_vector_search` target keys are undiscoverable, and a wrong key answers 403 | P2 | C9 | 0 / 3 | ♻️ (⚙️ only if the fix ships an enum on the input schema) | `com.etendoerp.go` | ⏳ open | Registered 2026-09-15 from the **2026-09-14 agentic probe harness** batch (work order [`docs/plans/2026-09-15-mcp-corrections-batch.md`](../plans/2026-09-15-mcp-corrections-batch.md); evidence in [`mcp-tests/findings/`](../../mcp-tests/findings/), eight files dated 2026-09-14; harness design [`docs/plans/2026-09-11-mcp-test-harness-design.md`](../plans/2026-09-11-mcp-test-harness-design.md)). **One row per cause, by deliberate decision** — the batch was first registered as a single source-based row and split the same day, because a five-clause ⚠️ scores 2.5 / 5 whether one clause or four are closed (§4, 2026-09-15). Batching is a delivery decision, registry rows are a measurement decision; the branch stays one branch. Cohort **C9** and this pointer are what hold the batch together. *What happens.* A DB Extended search-target key **is not a spec name**, and nothing on the MCP surface enumerates the valid keys — no list tool, no enum in the input schema, and `neo_discover` does not carry them. The only strategy left is guessing, which is what two probes did, using the spec names they knew. The guess then fails as `403 "Access denied to vector target"`, **indistinguishable from a genuine permission problem**: the agent cannot tell *this key does not exist* from *this key exists and your role cannot use it*, and the message offers no next step ([finding](../../mcp-tests/findings/2026-09-14-vector-search-targets-are-undiscoverable.md), work order §4/B1). *Why it compounds.* Every other NEO error observed in the same run does better — `unknown_filter_field` returns the `available` list plus a `hint`, `validation_error` returns `missingFields` with labels and selector availability. This is the one path that regressed to an opaque denial, i.e. outside the [IMP-5](imps/IMP-5.md)/[IMP-17](imps/IMP-17.md) envelope those items exist to impose. It also removes the fallback for **IMP-42**: the tool that exists precisely for fuzzy lookup is the one that cannot be used. Adjacent: **IMP-9** (`neo_discover` surface). *UI test (D22) is inconclusive and deliberately not claimed either way* — semantic search is not a window a person opens. The defect claimed is the **error contract**, not the denial: making the denial go away is not the fix. **Not known.** Whether DB Extended is installed, or has any search target configured in this tenant — **unchecked**. If it has none, the *denial* is correct and only the message and the undiscoverability are the defect; that does not invalidate the item (both branches lead to the same two requirements) but it decides whether a happy path is testable at all, so check it first. No valid key was ever found. Whether the 403 is emitted by the MCP layer or propagated from DB Extended is unknown, and it decides where the envelope has to be applied. `Done when:` two separable things hold — (a) the valid target keys are discoverable through the MCP, as an enum, a list, or via `neo_discover`; and (b) an **unknown** target is not reported as a permission denial but returns the IMP-5-shaped envelope naming what was wrong, what is available and what to do next, while a genuine permission denial stays distinguishable from it | [finding](../../mcp-tests/findings/2026-09-14-vector-search-targets-are-undiscoverable.md) · work order §4/B1 |
