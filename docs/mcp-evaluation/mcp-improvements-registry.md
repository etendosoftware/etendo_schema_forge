...
| Component | Weight | Normalization | 2026-08-05 | 2026-08-06 | 2026-08-10 | 2026-08-13 | Contribution |
|---|---:|---|---|---|---|---|---:|
| **M2** — first-call success rate | 30 | the percentage itself | 0 % ¹ | 40 % ² | 80 % ⁴ | **67 %** ⁵ | 20.1 |
| **M1** — calls-to-outcome ratio vs Holded | 30 | `100 / M1` (1.0× → 100) | 2.4× → 42 | 2.1× → 48 | 1.4× → 71 | **1.0× → 100** ⁶ | 30.0 |
| **Delivery** — weighted points earned | 25 | `earned / quota` | 29.5 / 73 → 40 | 29.5 / 97 → 30 ³ | 49.0 / 97 → 51 | **68.0 / 126 → 54** | 13.5 |
| **Coverage** — probe surfaces exercised (§2.5) | 15 | `probed / 6` | 2 / 6 → 33 | 6 / 6 → 100 | 6 / 6 → 100 | **6 / 6 → 100** | 15.0 |
| | | | **MARI = 28** | **MARI = 49** | **MARI = 73** → 70 ⁷ | | **MARI = 79** |
...
| **Today** (2026-08-13) — IMP-11 + IMP-12 + IMP-22 resolved, IMP-19 + IMP-21 + IMP-23 scored; IMP-28 + IMP-29 registered (no re-base) | **67 %** | **1.0×** | **68.0 / 126** | 6 / 6 | **79** |
| **Registry closed** — all 29 ✅, full probe surface, M1 at its target | 90 % | 1.2× | 113 / 126 | 6 / 6 | **88** |
...
| Surface | 2026-08-05 | 2026-08-06 | 2026-08-10 | 2026-08-13 | Note |
|---|---|---|---|---|---|
...
| `neo_widget` + the 8 report generators | ❌ | ✅ | ⚠️ | ✅ | B1–B2, B3–B5. **Not re-probed on 2026-08-10** (job A, nothing in the wave touched them) — the surface still counts as covered by the 08-06 evidence, but IMP-19's clauses are 08-06-fresh, not 08-10-fresh. **Re-probed live on 2026-08-13** (`generate_tax_report`, run report §5.3) — the 08-10 ⚠️ is closed |
...
**Totals (2026-08-13):** earned **68.0** of a known scope of **113** (C1 32/40 · C2 16.5/21 ·
C3 8.5/20 · C4 11/16 · C5 0/8 · C6 0/8) against a quota of **126** (§2.2) → the Delivery component of
MARI = **54** (§2.1). Verify the column sums before publishing; a `Pts` cell that disagrees with its
`Status` mark makes MARI unauditable.

**The quota was NOT re-based this run.** Registering IMP-28 and IMP-29 took known scope from 105 to
113, which still sits **13 points below** the quota, so the room check §3 demands was satisfied
before any status was promised and no amendment was needed. Reserve: 21 → 13.

> ⚠️ **Three `Pts` / `Status` mismatches are left standing on purpose — they are the human's call.**
> IMP-17 is ✅ but scored 0/3, IMP-18 is ⚠️ but scored 0/3, and IMP-24 is ⚠️ but scored 0/5. Correcting
> them to match their marks would add **7 points** (68.0 → 75.0, Delivery 54 → 60, MARI 79 → 80) with
> no code shipped, which is exactly the kind of movement this registry exists to prevent a run from
> awarding itself. They are reported in the 08-13 run report as owed by the human, not fixed here.
...
> 21 points below the quota (126) — **08-13 spent 8 of those points on IMP-28 and IMP-29, leaving 13.**
> The next run still has room to register what it finds, but not much: a single P1 plus a P2 would
> take it to 5, and two P1s would overrun. The
