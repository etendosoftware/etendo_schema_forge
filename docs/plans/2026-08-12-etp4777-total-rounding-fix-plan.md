...
6. **Reverse sync — editing the Exchange Rates tab directly updates the header (asked explicitly, not covered by the first pass above)** — created a fresh Draft Sales Invoice (10000022), contact "Tercero España", currency USD (rate 1.47 auto-applied), one line (Agua, 12,00 € × 1.47 = 17,64 $, tax 3,70 $, total 21,34 $). Opened the "Exchange Rates" tab and edited the `Índice` cell directly from `0.680272108844` to `0.5` (this is the doc→org rate, the inverse of the header's org→doc `eTGOCurrencyRate`). Result: the header's `Moneda` picker updated **immediately**, with no reload, from `USD — 1.47` to `USD — 2.00` (= 1/0.5, correctly inverted) — confirmed both visually and via the header API (`eTGOCurrencyRate: 2`). `grandTotalAmount` (21.34) and the Form panel's Subtotal/Impuesto/Total were **unaffected**, correctly, since the document-currency amount doesn't depend on the org-conversion rate. This exercises `DetailView.jsx`'s pre-existing `getSecondaryRowUpdateHandler` (lines 217-278, specifically the `st.key === 'exchangeRates'` branch at 268-271) — code this fix did not modify, but that shares the same `hook.refreshHeaderTotals` call this fix newly added a second call site for (§5 item 2). Confirmed still working correctly in both directions.

**Conclusion: no regressions found against ETP-4027/4028/4029, in either sync direction (header→tab or tab→header).** The one behavioral nuance found (item 3) is a pre-existing backend design characteristic (discount materialization timing), newly *visible* now that the Form panel stopped lying about the total — not something this fix introduced or should fix.

## 8. Collateral finding — list-view footer "sum" ignores currency (NOT part of ETP-4777, not fixed here)

Asked explicitly: "does the list view of each window show amounts correctly?" Per-row amounts: yes, confirmed extensively throughout §6/§7 (every row's "Imp. Total" always matched the Form/Grid/PDF for that specific document, in whatever currency that document has). The **footer total row** at the bottom of each list, however, is wrong whenever the visible rows mix currencies — confirmed on `sales-order` (footer showed `11.589,07 $`, the raw arithmetic sum of rows in both € and $ — e.g. `150,96 $ + 478,16 € + ... = 11589.07`, mislabeled with `$`) and `sales-invoice` (`11.373,90 $`, same pattern). Root cause, found by reading the code (not part of this fix, not modified):

```js
// DataTable.jsx:2018 — currency-blind sum
sums[col.key] = filteredData.reduce((sum, row) => sum + (Number(row[col.key]) || 0), 0);

// DataTable.jsx:1770-1772 — labels the sum with the FIRST row's currency, regardless of the rest
formatCurrency(filteredData[0]?.['currency$_identifier'], totals[col.key])
```

This lives entirely in `DataTable.jsx`'s generic footer-totals feature (`renderFooterRow`), used by every list window — nothing to do with `computeDocumentTotals`, `DocumentTotalsPanel`, or any file this fix touches. It's also not something this session's testing introduced: `purchase-order`'s original seed data already had a mixed-currency row (`1000011`, 40,91 $, alongside all-€ siblings) before any test data was added this session — this fix's USD test documents just made the bug easier to notice while reviewing list screenshots, since more lists ended up mixed. **Not in scope for ETP-4777 — flagging as a separate, real bug** (`DataTable.jsx` needs to either group-sum by currency and show N totals, or suppress the footer total entirely when the visible rows don't share one currency) for a follow-up ticket.
