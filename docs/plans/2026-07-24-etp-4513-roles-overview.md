...
>
> **Final status (2026-07-26) — pipeline complete, DEV → REVIEW → QA → DOCS:** Both PRs (`etendo_schema_forge` #958, `com.etendoerp.go` #772) are now fully through all four phases. REVIEW (Alex) approved both, with the documentation-freshness gap noted above closed before QA started. QA (Sentinel) approved both, adding regression coverage on top of DEV's own tests: two backend JUnit cases in `SFRolesOverviewTest` (`testCallerIsAGoClientRoleButNotAdminIsStillDenied`, `testRoleWithNoUsersAndNoWindowAccessDegradesGracefully` — commit `348be82b`) and one frontend Vitest case in `RolesOverviewPage.vitest.jsx` asserting the Edit dialog is truly read-only (no form, no editable fields, single Close button — commit `74d173a87`). DOCS (Sage) then verified and refreshed both docs to reflect QA's additions: `com.etendoerp.go/docs/neo-headless.md` §9's `SFRolesOverviewTest` row now calls out the two new cases specifically, and this repo's `docs/generated-custom-windows/app-shell-functional-flows.md` §6b's "Automated evidence" bullet now mentions the read-only-dialog assertion. See "QA Follow-ups (informational, non-blocking)" below for two findings Sentinel flagged but explicitly did not fix. Both PRs remain stacked on `feature/ETP-4520`/PR #944 and will need re-targeting to `epic/ETP-3504` once that PR merges — no other action is pending on this ticket.
...
## QA Follow-ups (informational, non-blocking)

Sentinel (QA) approved both PRs with no blocking findings, but flagged two informational items worth recording here as follow-ups rather than fixing now:

- **`e2e/tests/flows/roles-overview.mocked.spec.js` is flaky under default Playwright parallelism** (4 workers) — a timing race between `page.route()` fixtures and `mockFetch.js`'s async `window.fetch` override. This is the same class of issue already known for `SFWindowAccessMap` (see "Known Limitation Discovered" above), now confirmed to generalize to `SFRolesOverview` too. Stable at `--workers=1`. Not currently CI-gated, since no E2E workflow exists yet.
- **A "direct navigation to `/roles` as a denied non-admin" E2E deep-link test was deliberately NOT shipped.** `mockFetch.js`'s `handleRolesOverviewRequest()` has no admin branching (unlike `handleWindowAccessMapRequest()`), so the scenario can't be reliably forced via `page.route()`/`addInitScript` once the app's own mock install takes over. The equivalent coverage exists instead split across two other tests: the backend JUnit denial test (`SFRolesOverviewTest#testCallerIsAGoClientRoleButNotAdminIsStillDenied`) and the frontend empty-state Vitest test. Sentinel left the full explanation as a comment directly in `roles-overview.mocked.spec.js` — see that file, not duplicated here.
