...
**Update (2026-08-31): the production update runs EVERY day, and it runs BEFORE the
day's merge block.** `main` is the **production** environment and `develop` is **staging**,
so this is a real deploy window, not just bookkeeping. Both repos (functional
...
> **Two changes landed in sequence here.** 2026-08-30 retired the epic tier, collapsing the
> old 3-step order (`epic → develop`, `develop → main`, `feature → epic`) into two steps.
> 2026-08-31 then made the cadence **daily** and put the production update **first**, so a
> block always lands on top of an already-promoted `main`. `epic → develop` is gone for good.

### Who does what

| | Create the PR | Approve | Merge |
|---|---|---|---|
| `develop → main` (daily, first) | **Clerk** (on request) | Team reviewers | **Human only** |
...
Each block day gets **one Jira task** (`Merge Block DD/MM`, type Task, filed under
