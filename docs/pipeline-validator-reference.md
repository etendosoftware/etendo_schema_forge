...
| F18 | BLOCK (skips when slice absent) | `artifacts/<window>/generated/web/<window>/labels.js` is present **and** stale — it does not match the per-window field-label slice recomputed from the current `contract.json` columns × the locale dictionaries (`packages/app-shell-core/src/locales/*.json`). Per-window slices are **gitignored build-time artifacts** (ETP-4300, not committed), so F18 skips when `labels.js` is absent (the normal CI state); it acts as a build-time sanity check when a slice exists on disk. Translation-coverage gaps (rendered columns with no dictionary label) are NOT an F18 concern — they are surfaced by `slice-labels.js --check`, not the validator. | Re-run `node cli/src/slice-labels.js --window <window>` to regenerate the label slice. |
