...
**Fix:** replace the trailing `**` with `{/**,}**` — `page.route('**/sws/neo/<entity>/header{/**,}**', ...)`. This brace-alternation glob matches both branches: `/**` (any sub-path, e.g. `/header/{id}`, `/header/action/post`) and the empty branch followed by `**` (bare `/header` and `/header?query=...`, preserving the original behavior). Patterns that already end in `/**` (e.g. the generic `**/sws/**` catch-all) are unaffected — a `/` immediately before `**` already crosses path separators correctly. Patterns intentionally scoped to query-string-only variants (ending in a literal `?**`, e.g. `**/sws/neo/session?**`) are also a different, deliberate case — leave them as-is unless the endpoint also needs a sub-path.

This was diagnosed against a large batch of pre-existing `.mocked.spec.js` failures (55 tests across ~22 files) that had nothing to do with the PRs under test — the bug pre-dated them. Reach for this pattern by default in any new `page.route()` call whose real endpoint can receive a sub-path (detail GET by id, `/action/...`, `/defaults`, `/selectors/...`).
