...
- Payment-state reactions are clearly visible on the detail page. `Process Payment` is exposed only when status is `RPAP`, while `Reverse Payment` is exposed only when status is `RPPC`, `RPR`, or `RDNC`. The page also hides the plain delete button once the payment is considered complete (`hideDeleteWhenComplete: true`).
- A **Remove Payment** action is available from the detail-view kebab (`window.menuActions`, key `removePayment`) for every status except `RPVOID` — this covers both draft and confirmed/deposited payments, unlike the plain delete button. It fires the `eTPRRemovePayment` AD process (`RemovePayment`, `OBUIAPP_PROCESS_ID = FB79E902A5384754990AD145F6CAC9FB`), which internally reactivates and then removes the payment so it is safe to run regardless of processed state. The same field already drives the row-level "Delete" action in the list view (`PaymentHeaderTableBase.jsx`), gated by the identical `status != 'RPVOID'` rule — this menu action brings that capability to the form/detail view, which previously had no path to remove a confirmed payment.
...
6b. Open the detail-view kebab (⋮) on a draft payment and confirm **Remove Payment** is listed; confirm it is still listed after processing to `RPR`/`RDNC`/`RPPC`, and confirm it disappears once the payment is `RPVOID`.
...
- There is no dedicated payment-in UI test in `tools/app-shell` covering the specialized create flow, payment-state actions, or allocation panels. This includes the new **Remove Payment** kebab action (ETP-4479) — no Vitest/Playwright coverage exists yet for its status gating or the `eTPRRemovePayment` process call.
