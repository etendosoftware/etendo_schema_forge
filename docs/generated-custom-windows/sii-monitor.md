...
`decisions.json` declares `window.readOnly: true`, so **all 11 entities are restricted to
`GET` + `GETBYID`** on `ETGO_SF_ENTITY` (`ISPOST`/`ISPUT`/`ISPATCH`/`ISDELETE` = `N`). This is
