...
6. Open the saved record, add one or more lines, and confirm Product, Movement Quantity, and Storage Bin can be entered. Confirm there is no **Line No.** column and that "Movement Quantity" fits on one line. Confirm the Storage Bin column/field label reads "Warehouse" / "Almacén". Confirm the Movement Quantity field rejects a negative value.
6b. Before adding any line, confirm the lines tab shows the "No lines yet" / "+ Add lines" empty state.
...
- `artifacts/internal-consumption/decisions.json` `window.labelOverrides` and `entities.lines.fields.movementQuantity.min` show the ETP-5039 label and validation changes; `artifacts/internal-consumption/contract.json` confirms `validation.minimum: 1` on `movementQuantity` and the absence of `displayFromCatalog` on `storageBin`.
- `artifacts/internal-consumption/custom/InternalConsumptionBottomPanel.jsx` exposes the shared `LinesEmptyState` (from `@/components/contract-ui`) as its `linesEmptyState` static.
