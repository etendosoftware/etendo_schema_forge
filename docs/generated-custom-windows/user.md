...
**Company user invitations (ETP-4894):** The User window provides a dedicated company user invitation banner and email-only modal dialog (`InviteUserDialog`). The administrator must first create the matching `AD_User` in the organization and assign its roles. Creating an invitation then sends a transactional `company-invitation` email to the invitee containing a secure, expiring link (`/invite?token=...`). The invitee resolves the invitation on a public acceptance page (`InviteAcceptancePage`):
...
- The user-create handler derives a unique ERP username from the normalized email, appending the sanitized client name when that email already owns another client; the email remains the platform-account identity. This and account provisioning are implemented in `UserRoleAssignmentHandler`, with the username convention shared with onboarding's `EtendoGoJwtSupport.buildClientUsername`. Each password-setup invitation is persisted in `ETGO_Invitation` with its account, ERP user, hashed token, expiry, and lifecycle status (`PENDING`, `SENT`, `DELIVERY_FAILED`, or `ACCEPTED`). The invitation calls the existing server-side `reset-password` contract; it does not expose a browser email-provider payload.
