...
- A **Remove Payment** action is available from the detail-view kebab (`window.menuActions`, key `removePayment`) for every status except `RPVOID` — draft and confirmed/deposited payments alike, unlike the plain delete button which is hidden once the payment is complete (`hideDeleteWhenComplete: true`). It fires the `eTPRRemovePayment` AD process (`RemovePayment`, `OBUIAPP_PROCESS_ID = FB79E902A5384754990AD145F6CAC9FB`), which internally reactivates and then removes the payment, so it is safe regardless of processed state. The same field already drives the row-level "Delete" action in the list view (`PaymentHeaderTableBase.jsx`), gated by the identical `status != 'RPVOID'` rule — this menu action brings that capability to the form/detail view.
...
6b. Open the detail-view kebab (⋮) on a draft payment and confirm **Remove Payment** is listed; confirm it is still listed after processing to a deposited/cleared status, and confirm it disappears once the payment is `RPVOID`.
...
- No payment-out-specific browser or node test was found that proves end-to-end outgoing-payment lifecycle behavior, multi-currency recalculation, credit usage, posting effects, or exposure of the contract-defined secondary surfaces. This includes the new **Remove Payment** kebab action (ETP-4479) — no Vitest/Playwright coverage exists yet for its status gating or the `eTPRRemovePayment` process call.
