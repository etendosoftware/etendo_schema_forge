...
20. On `/sales-order`, open the row kebab of a completed order with pending shipment or invoice work and confirm `Gestionar envío / factura` is listed; trigger it and verify it opens the same create-docs modal used from the detail page, and that the app does not crash to a blank page (regression: ETP-5295). On a fully fulfilled order, trigger the same kebab item and confirm the launcher closes silently without opening a modal. On a completed order with pending work, check the shipment and/or invoice boxes in the launcher and submit — confirm the same result popup used by "Confirmar" appears (titled "Documentos creados" / "Documents created") with clickable links to the newly created document(s), instead of the launcher closing silently with no feedback.
