...
- The 4-digit suffix field in the new-subaccount modal shows the **highest existing suffix** under the selected parent prefix as its placeholder (e.g. `0004` when `20000001..20000004` already exist), computed client-side from the already-loaded account list; falls back to the generic `0000` placeholder when nothing exists yet under the prefix. Purely a hint — does not pre-fill or validate the field.
...
20. Open "New Sub-account" for a parent prefix that already has children (e.g. `2000`) and confirm the suffix field's placeholder shows the highest existing 4-digit suffix under that prefix, not the generic `0000`. Pick a prefix with no children yet and confirm it falls back to `0000`.
