...
20. Open "New Sub-account" for a parent prefix that already has children (e.g. `2000`) and confirm the suffix field's placeholder shows the next available 4-digit suffix under that prefix (highest existing + 1), not the generic `0000`. Pick a prefix with no children yet and confirm it falls back to `0000`.
21. Try to create a subaccount whose full 8-digit code already exists and confirm the toast reads "Account `<code>` already exists" (localized), not the generic "Could not create sub-account" message.
22. Open `/chart-of-accounts` and confirm no small count badge appears next to the "Chart of Accounts" title — other list windows (e.g. General Ledger Configuration) still show theirs, confirming this is a per-window opt-out, not a global change.
