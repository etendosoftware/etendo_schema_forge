...
- **ETP-5350 / IC-17 follow-up:** toggling an existing saved contact is a preview until Save. The hidden identity values are kept as local drafts: Company → Person clears the active legal-name payload but restores the prior person-name draft, and Person → Company clears the active person-name payload but restores the prior legal-name draft. Toggling back before Save restores the value originally loaded or typed for that mode. Saving persists only the active type's fields; canceling discards both local drafts.
...
**Type-aware identity import (ETP-5350 / IC-17).** For an **Empresa**, Razón Social is used when supplied; otherwise both Nombre and Apellido are concatenated into Razón Social, and those person-only columns are omitted from the business-partner payload. For a **Persona**, both Nombre and Apellido are required during the review step; any Razón Social column is ignored and the technical `name`/`searchKey` values are derived from the person name. This keeps fields belonging to the inactive contact type out of persisted imports while still satisfying the mandatory `C_BPartner.Name` column.
