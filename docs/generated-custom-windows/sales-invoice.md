...
- **Adjuntos sections inside the SIF tab (ETP-4888):** SII and TBAI attach their outbound/response XML to a fiscal sub-record — `aeatsii_facturas` and `tbai_syncinvoice` respectively — not to the invoice (`C_Invoice`) itself, so the window's generic **Attachments** tab (which is keyed on the invoice's own `tableName`/`recordId`) can never reach those files. Verifactu's AEAT-response leg has the same problem via `etvfac_c_invoice_verifactu`; its outbound-send leg already attaches directly to `C_Invoice` (via `GenerateRF#saveInvoiceXML`) and already shows up in the standard Attachments tab, so it is deliberately NOT duplicated here. To close this gap, each SIF panel now renders a read-only "Adjuntos" section (view/download only — no upload/delete, since these are the actual signed XML sent to AEAT and the government's own response, not ad-hoc business attachments) pointed at the invoice's own sub-record:
  - **SII panel:** lists attachments for the invoice's `aeatsii_facturas` row.
  - **Verifactu panel:** lists attachments for the invoice's `etvfac_c_invoice_verifactu` row (the AEAT-response leg only).
  - **TBAI:** since ETP-4401 removed the TBAI field panel entirely, ETP-4888 reintroduces a new, deliberately minimal TBAI rail/panel whose ONLY content is this Adjuntos section, for the invoice's `tbai_syncinvoice` row — no date/type/checkbox fields were brought back.
  Each panel's sub-record id (`aeatsiiFacturaId`, `invoiceVerifactuId`, `tbaiSyncInvoiceId`) is resolved server-side (most recent active row, `ORDER BY created DESC`) and stamped onto the invoice header GET response by `SalesInvoiceHeaderHandler#afterHandle` via the new `SifSubRecordAttachments` helper (`com.etendoerp.go` — mirrors `InvoiceExemptTaxes`/`TbaiSyncStatusInjector`'s fail-safe pattern: a fiscal module that isn't installed simply omits that key). The frontend section (`tools/app-shell/src/windows/custom/shared/SifAttachmentsSection.jsx`) reuses the same generic attachments hook/table the classic Attachments tab uses (`useAttachments` + `AttachmentsTable`, `tools/app-shell/src/components/attachments/`) — no new fetching logic — and renders nothing when the sub-record id hasn't been resolved yet (e.g. the invoice was never sent to that target). `aeatsii_payment` and `aeatsii_cash_receipt_data` were investigated and confirmed to key on `fin_payment_id`/`aeatsii_cash_receipt_id` rather than `c_invoice_id` — they back the Payment/Cash Receipt SII windows, not this one, and are out of scope here.
