...
- **Visibility:** visible in the **Settings** menu as **Business Partner Category** (AD window ID `192`)
...
- **Java handler not yet implemented.** `decisions.json → entities.accounting.javaQualifier` is set to `"businessPartnerCategoryAccountingHandler"`, matching the pattern used by `ProductCategoryAccountingHandler.java` in `com.etendoerp.go`. **No such handler exists yet.** `C_AcctSchema_ID` has no native AD default value, so — until this handler is built — creating a new Accounting row for a business partner category will not auto-populate the accounting schema, mirroring the exact gap already documented for Product Category before its handler was written. This must be built by a Schema Forge Developer before the Accounting tab is fully functional for record creation.
...
7. Attempt to create a new Accounting row and confirm whether `accountingSchema` populates automatically — expected to fail until `businessPartnerCategoryAccountingHandler` is implemented (see Gap assessment).
