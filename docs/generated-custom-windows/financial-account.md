...
- **A group states its type, and a near match previews the movement it will create (ETP-4965, QA round).** Until this round only rule groups were labelled, so an exact suggestion and a within-tolerance one looked identical even though the second posts an accounting entry; and the difference movement itself was invisible until it appeared in Movimientos.
  ...
  - **Error copy.** `applySuggestions` answers **201 even when every group is rejected** — the reason travels inside `results[]` — so the modal's `catch` never runs and its old two-counter reduction produced a bare "Error al aplicar la conciliación". It now reports the backend's own `results[].error.message`, and when *every* failure is `GL_ITEM_REQUIRED` it names that cause explicitly (`financeReconcileAutomatchToastNoGlItem`), as a warning when part of the batch still went through and an error when nothing did. `needsGlItem` is also reset when the modal opens; it used to latch, so the "Editar cuenta" remedy kept showing for a problem already fixed.
