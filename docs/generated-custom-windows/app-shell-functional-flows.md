...
- **Failure or edge behavior:**
  ...
  - **Open question, not resolved here:** whether the real `matrix.categories` includes the windowless `General`/Inicio/Favoritos/Copilot-style rows the ETP-4907 reference screenshot shows, or whether the frontend needs to overlay those itself — the sibling, not-yet-merged `feature/ETP-4906` branch's `UserRolesTab.jsx` hardcodes an equivalent `GENERAL_ROWS` client-side for exactly this reason (those rows have no `AD_Window_ID` to originate from). Not implemented on this page pending backend confirmation.
- **Automated evidence:**
...
- **Manual verification path:**
  ...
  2. Confirm the matrix below renders every category with the correct tri-state pill per cell, and that both "Contactos" rows (Comercial and Inventario) render independently with their own access.
