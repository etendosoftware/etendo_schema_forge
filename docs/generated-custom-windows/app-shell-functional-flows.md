...
- **Failure or edge behavior:**
  ...
  - An account that predates ETP-4798, or one whose confirmation mail could not be sent, is neither verified nor pending: it sees no wall and hits no gate. The flow fails open rather than locking users out over a misconfigured `etendo.go.app.baseUrl` or provider — which is why the routing predicate is `emailVerificationPending` and never `!emailVerified`.
