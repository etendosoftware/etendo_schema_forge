...
- **Main path behavior (ETP-4907 redesign):**
  ...
  - Data comes from `useRolesOverviewData()` (`tools/app-shell/src/pages/roles/useRolesOverviewData.js`) — **currently backed by mock data** matching the ETP-4907 reference screenshot's numbers, isolated behind a single `fetchRolesOverviewData()` swap point for the real `com.etendoerp.go` endpoint (being built in parallel on that repo's own `feature/ETP-4907` branch; contract not final yet — see that module's file-level JSDoc). This REPLACES the old (ETP-4513) per-role card list that used to fetch `GET /sws/neo/rolesoverview` (`lib/rolesApi.js`'s `fetchRolesOverview()`); that endpoint/module is untouched and still used elsewhere (the sibling, not-yet-merged `feature/ETP-4906` branch's `UserRolesTab.jsx`).
  - Each matrix row's key is `${category}::${windowKey}` (`useRolesOverviewData.js`'s `buildRowKey`), not the window name alone — the mock data has a real duplicate-window-name case ("Contactos" appears in both the `Comercial` and `Inventario` categories with different access per role) that a name-only key would silently collide.
...
- **Failure or edge behavior:**
  - A non-array/missing `cards`/`matrix` field is treated as empty, not a crash.
  - An empty `cards` array renders a dedicated no-access card — a defense-in-depth fallback for direct/deep-link navigation around the menu gate; the real enforcement point will be the ETP-4907 backend endpoint once it lands (mirroring `SFRolesOverview.java`'s existing "deny silently with an empty payload" convention).
...
- **Automated evidence:**
  - `tools/app-shell/src/pages/__tests__/RolesOverviewPage.vitest.jsx` covers the loading/error/no-access states, the 5 summary cards (icon, curated name, user-count badge, window-count line), and the matrix (category grouping, per-role tri-state cells, the duplicate-"Contactos"-row edge case rendering both rows with their own independent access).
  - `tools/app-shell/src/pages/roles/__tests__/useRolesOverviewData.vitest.js` covers the data-shaping helpers directly: `buildRowKey`/`flattenMatrixRows` producing distinct keys for the two same-named "Contactos" rows, and `sortByRoleOrder`.
  - `e2e/tests/flows/roles-overview.mocked.spec.js` (ETP-4513) predates this redesign and asserts against the OLD per-role-card-list DOM shape — it is now stale and needs a follow-up pass (flagged for QA/Tester, not fixed as part of this ticket).
- **Manual verification path:**
