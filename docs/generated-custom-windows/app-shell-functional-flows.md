...
- **User goal / entry point:** View GOClient's 5 fixed roles — their window/user counts and, per window (grouped by menu category), which roles can reach it and at what tier — at `/roles`.
- **Main path behavior (ETP-4907 redesign):**
  - `RolesOverviewPage` renders 5 summary cards (`RoleSummaryCard`, one per role: icon, curated i18n name, a user-count badge, and a large "N Ventanas" line) followed by a full-width window x role access matrix (`RolesAccessMatrix`), rows grouped by category (`General`/`Comercial`/`Ventas`/`Inventario`, translated via `useMenuLabel()` against the same literal section names `menu.json`'s groups already use).
  - Each matrix cell is tri-state, rendered by the shared `AccessTierPill` component (`tools/app-shell/src/components/AccessTierPill.jsx`): green check pill (`full`), amber "Solo lectura" pill (`readOnly`, reusing the pre-existing `accessTierReadOnly` i18n key), or a plain gray em-dash (`none`).
  - This page is a **hand-built standalone page** (no `decisions.json`/`artifacts/roles/` pipeline artifact, not in `windows/registry.js`), routed via `runtime-routes.jsx`'s `lazyRoute('roles', RolesOverviewPage)` — the general convention for a screen with no corresponding AD Window is documented in `../window-templates.md` § "Standalone Pages (No AD Window)".
  - Data comes from `useRolesOverviewData()` (`tools/app-shell/src/pages/roles/useRolesOverviewData.js`), which calls the SAME `GET /sws/neo/rolesoverview` (`lib/rolesApi.js`'s `fetchRolesOverview()`, unchanged since ETP-4513) the OLD per-role-card-list version of this page used — no new endpoint was needed. The backend developer found this independently while this frontend work was in progress and extended `SFRolesOverview.java` in place with `windowCount`/`matrix`/`roleSource` (`com.etendoerp.go`'s `feature/ETP-4907`, commit `c3424a58`, not yet pushed). `adaptCards`/`adaptMatrix` in `useRolesOverviewData.js` translate the real response (`roles[]` with `windowCount`/`userCount`/`isClientAdmin`/`roleSource`, plus a new top-level `matrix.categories[]`) into this page's card/matrix shape, normalizing the backend's hyphenated `'read-only'` tier to this app's camelCase `'readOnly'` in one place (`normalizeTier()`).
  - Each matrix row's key is `${category}::${windowId}` (`useRolesOverviewData.js`'s `buildRowKey`), using the backend's real per-window `id` rather than its translatable `name` — the real data has a duplicate-window-NAME case ("Contactos"/Business Partner appears in both the `Comercial` and `Inventario` categories with different access per role) that the backend already disambiguates as two separate entries with their own `id`, so keying by id sidesteps the collision rather than needing a name-based composite key.
  - Role icon/order assignment uses `resolveRoleKind(role)` (`isClientAdmin` for Admin — confirmed name-independent by the backend developer — else the fixed English `AD_Role.name` for the 4 template roles, regardless of `roleSource: 'tenant'|'systemTemplate'`), since `roles[].id` is a real per-tenant value with no stable meaning across tenants, while the matrix's `access` map IS keyed by that same real `id`.
  - `make dev-mock`/E2E's backend-less fallback (`lib/mockFetch.js`'s `handleRolesOverviewRequest()`) was updated alongside this module to return the new `windowCount`/`isClientAdmin`/`roleSource`/`matrix` fields — this is the sanctioned "no live backend" mock path in this codebase; `useRolesOverviewData.js` itself carries no mock data anymore (moved into this feature's own test fixtures).
  - Role display order (cards and matrix columns, left to right): Admin, Sales, Purchasing, Finance, Inventory — matches the ETP-4907 reference screenshot, which differs from the Admin/Finance/Sales/Purchasing/Inventory order used elsewhere (`SFRolesOverview.java`'s `GOCLIENT_ROLE_IDS`, the old version of this page).
  - The matrix's `General` category (Inicio/Dashboard, Favoritos, Copilot) is a hardcoded client-side overlay (`RolesAccessMatrix.jsx`'s `GENERAL_ROWS`, always rendered first, always full access for every role), NOT sourced from `matrix.categories` — confirmed against the sibling `feature/ETP-4906` branch's `UserRolesTab.jsx` `GENERAL_ROWS` precedent that none of these 3 is a real AD window (`ETGO_SF_SPEC`-backed), so `SFRolesOverview` will never return them. Reuses that same branch's exact i18n keys (`userRolesTabDashboardRow`/`userRolesTabFavoritesRow`/`userRolesTabCopilotRow`) so the two key sets de-dupe cleanly once that branch merges instead of drifting apart.
  - There is no create/edit/delete affordance anywhere on the page; this remains a read-only view. The Topbar title/breadcrumb ("Configuración / Roles") is now wired via `useSetPageMeta` (previously this page rendered its own inline heading with no Topbar breadcrumb at all).
  - The menu entry that routes here (`menu.json`'s `roles` item, under "Configuración") is unchanged by this redesign — still gated client-side by `"capability": "isAdminOrClientAdmin"`, resolved by `registry.js`'s `filterMenuGroupsByAccess` against the `SFWindowAccessMap` capability of the same name (`com.etendoerp.go/docs/neo-headless.md` §8b).
- **Failure or edge behavior:**
  - A non-array/missing `roles`/`matrix` field in the response is treated as empty, not a crash.
  - An empty `cards` array (returned by the backend for a non-admin/no-role caller, or for direct/deep-link navigation around the menu gate) renders a dedicated no-access card — a defense-in-depth fallback, since `SFRolesOverview.java` is the actual enforcement point.
...
- **Automated evidence:**
  - `tools/app-shell/src/pages/__tests__/RolesOverviewPage.vitest.jsx` covers the loading/error/no-access states, the 5 summary cards (icon, curated name, user-count badge, window-count line), and the matrix (category grouping, per-role tri-state cells).
  - `tools/app-shell/src/pages/roles/__tests__/useRolesOverviewData.vitest.js` covers the data-shaping/adapter logic directly: `normalizeTier` (hyphenated `'read-only'` → `'readOnly'`, everything else → `'none'`), `resolveRoleKind`, `buildRowKey`/`flattenMatrixRows` producing distinct keys for two same-NAME "Business Partner" rows via their different `windowId`s, `sortByRoleOrder`, and the hook's adaptation of a realistic `{roles, matrix}` payload (mocking `fetchRolesOverview`).
  - `tools/app-shell/src/pages/roles/__tests__/RolesAccessMatrix.vitest.jsx` covers the hardcoded `General` overlay specifically: always present even with an empty real matrix, all 3 rows full-access for every role unconditionally, and rendered before the real `matrix.categories` groups in document order.
  - `e2e/tests/flows/roles-overview.mocked.spec.js` (ETP-4513) predates this redesign, asserts against the OLD per-role-card-list DOM shape (`RolesOverviewPage__role-*`/`RolesOverviewPage__window-*` test ids that no longer exist), AND stubs the OLD `{roles: [...]}`-only response shape — now stale on two independent axes and needs a full follow-up pass (flagged for QA/Tester, not fixed as part of this ticket).
- **Manual verification path:**
  1. Open `/roles` while authenticated as a GOClient Admin (or other client-admin) role and confirm all 5 summary cards render in Admin/Sales/Purchasing/Finance/Inventory order with the correct window/user counts.
  2. Confirm the matrix below renders every category with the correct tri-state pill per cell, that both "Contactos" rows (Comercial and Inventario) render independently with their own access, and that the first section is a hardcoded "General" group (Inicio/Favoritos/Copilot) with every cell full-access ("✓") for every role.
  3. Confirm the Topbar shows the "Roles" title and "Configuración / Roles" breadcrumb.
