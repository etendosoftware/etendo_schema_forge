...
- Independent accounting date (ETP-4531): `accountingDate` (`DateAcct`) is now a full editable header field (own default `@#Date@`, principal section, locked once `Posted='Y'`) — previously hidden (`form: false`). Per classic AD metadata, `C_Invoice.DateInvoiced` carries `AD_Column.AD_Callout_ID = com.etendoerp.sif.general.callouts.SifInvoiceOperationDateCallout`, which extends `org.openbravo.erpCommon.ad_callouts.SE_Invoice_AccountingDate` and auto-fills `dateAcct` from `dateInvoiced`. `com.etendoerp.go`'s `NeoCalloutService`/`NeoCalloutEndpoint` execute this same classic callout server-side, so changing `invoiceDate` today still overwrites `accountingDate` behind the scenes — a `PurchaseInvoiceHeaderHandler#afterCallout` guard (mirroring the existing `blockCalloutCurrencyUpdate`/ETP-4029 pattern) is required in `com.etendoerp.go` to strip that side effect; tracked as a backend follow-up.
