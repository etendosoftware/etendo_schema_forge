...
- Independent accounting date (ETP-4531): `accountingDate` (`DateAcct`) is now a full editable header field (own default `@#Date@`, principal section, locked once `Posted='Y'` — deliberately `Posted` rather than this window's dominant `Processed='Y'` pattern, since `Processed='Y'` alone would also lock the date during a failed-posting-attempt state where it may still need correcting before retry) — previously hidden (`form: false`). Per classic AD metadata, `C_Invoice.DateInvoiced` carries `AD_Column.AD_Callout_ID = com.etendoerp.sif.general.callouts.SifInvoiceOperationDateCallout`, which extends `org.openbravo.erpCommon.ad_callouts.SE_Invoice_AccountingDate` and auto-fills `dateAcct` from `dateInvoiced`. `com.etendoerp.go`'s `NeoCalloutService`/`NeoCalloutEndpoint` execute this same classic callout server-side. The auto-fill side effect is neutralized on every path: `PurchaseInvoiceHeaderHandler#afterCallout` strips a callout-driven `accountingDate` update when the trigger field isn't `accountingDate` itself (mirrors the existing `blockCalloutCurrencyUpdate`/ETP-4029 pattern), and `NeoDefaultsCascadeHelper#processCalloutForField` applies the same strip generically to the GET `/defaults` (new-record bootstrap) and POST create cascades, which the `afterCallout` hook alone does not reach. Separately, promoting `accountingDate` to editable also surfaces its own callout — `C_Invoice.DateAcct` carries `SE_Invoice_TaxDate`, which only fires when the (unrelated, `taxDate`-not-null) formState precondition holds; `taxDate` is `visibility: discarded` in this window, so it never fires in practice — verified harmless, no change needed.
