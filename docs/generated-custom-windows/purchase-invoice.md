...
- **Create a product from the line's product selector (ETP-5254):** the product lookup drawer opened from a line now shows a pinned `+ Crear producto` row at the top. It opens a two-phase popup that renders the Products window's own form and primary-tab strip (same helper, same labels, not a lookalike). `Crear` saves the product; the popup then reveals that window's own **Precio** and **Adjuntos** panels for the new record, so its tariff price can be set on the spot — those panels need a saved record, which is exactly why creation happens in two steps. Closing the popup (`Completado`, the X, the overlay or Escape) is what selects the new product in the line; cancelling is only possible before it is created, and then the invoice is untouched. The line still arrives at **price 0** whenever the price step is skipped, or the price is set on a tariff other than the document's — the user then types it on the line, exactly as for any product with no row in that tariff. The popup creates no cost line and says nothing about cost — that rule belongs to the Products window, which states and enforces it there (ETP-5245); a stockable product created here therefore still needs its cost set in that window. The create row is reachable by pointer and Tab, not through the arrow-key ring. Full mechanism, the two phases, the seven in-scope specs and the known limitations: `docs/ui-customization.md` section 19.
