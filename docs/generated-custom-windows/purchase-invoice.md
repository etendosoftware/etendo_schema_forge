...
Before this change, a purchase invoice offered TBAI/Batuz sending (button, popup, badges, SIF tab, SIF bottom-panel block) whenever the org's fiscal profile was `tbai` or `sii+tbai`, regardless of which Basque territory the active TBAI configuration targeted. TicketBAI (Batuz) only applies to Bizkaia; Álava and Gipuzkoa use their own separate regimes, so a purchase invoice under those territories must never show TBAI. Sales invoices are unaffected — TBAI stays always-eligible for sales regardless of territory.

**Where the rule itself lives.** The territory-aware signature `getInvoiceFiscalTargets(specName, profile, territory)` (`tools/app-shell/src/windows/custom/shared/fiscalTargets.js`) and its `showTbaiForDoc = isSales || (isPurchase && territory === 'BIZKAIA')` gate are owned by **ETP-5027**, which generalized them beyond the two invoice specs to `sales-order`/`purchase-order` as well and documented the Java evidence behind both halves of the matrix (VERI\*FACTU is sales-only; Batuz/LROE is the only purchase path, Bizkaia-only). See the ETP-5027 note on the "Line-level tax needs SIF configuration shortcut" bullet under **Interaction model** above for that evidence — it is not repeated here.

**What ETP-5087 contributes** is the threading of `territory` through this window's consumers, so the gate is actually reached on every purchase-invoice surface rather than only where it was first applied.

The territory value comes from `tbaiRecord?.etsgSifTerritory` (via `useFiscalConfig(orgId, apiBaseUrl)`, the same hook that already returns `profile`) and is threaded through every consumer of `getInvoiceFiscalTargets`/`getPendingSifTargets`:

- `PurchaseInvoiceHeaderTable.jsx` — list-level SII/TBAI badge gating.
- `SendToSifButton.jsx` — detail-topbar and preview-modal `Enviar a SIF` button (via `getPendingSifTargets(specName, profile, data, territory)`).
...
`territory` defaults to `null` everywhere it is threaded, so a missing/unresolved TBAI config for the org (no `tbaiRecord`, or a territory other than `BIZKAIA`) is the same as "not Bizkaia" — purchase TBAI stays hidden, never fails open.

**Org resolution fix (follow-up, ETP-5087):** the territory gating above is only meaningful if `useFiscalConfig(orgId, apiBaseUrl)` is asked about the RIGHT org. Before this follow-up, `SendToSifButton.jsx`, `useSifFieldPatcher.js`, and `useInvoicePreview.js` all resolved `orgId` from the top-nav org selector (`useAuth().selectedOrg`) instead of the invoice actually being viewed/edited. Since the selector reflects whatever org the user is currently *browsing* — independent of which org the open invoice belongs to — a mismatch silently fetched the wrong (or no) TBAI/SII config, making the Bizkaia territory check above see the wrong territory or none at all.
...
`TaxSifField.jsx`'s `orgId` prop resolution (prefer an explicit prop, fall back to `selectedOrg`) was already correct as documented in its own docstring — it renders inside the standalone `tax` window, where the tax record's own org is `*` (never a legal entity), so `selectedOrg` is the only sensible source there, not a bug of this kind.

### List Batuz column — real sync state with a send-flag fallback, synchronous global visibility (ETP-5087)

The purchase-invoice list shows a **Batuz Status** column (`invoiceList.col.tbaiStatusPurchase` — "Estado Batuz" / "Batuz Status", deliberately NOT the generic `invoiceList.col.tbaiStatus` that sales-invoice uses, because a purchase invoice's TBAI target is always Bizkaia-gated Batuz, never the generic TicketBAI scheme).

**Where the data comes from — two sources, in strict priority order.** The cell renders:

```js
<FiscalStatusBadge status={row.tbaiSyncEstado ?? (isSent(row.tbaiIssent) ? 'Enviada' : 'Pendiente')} />
```

1. **Primary — `row.tbaiSyncEstado`.** The real outcome of the submission to Batuz (`Recibido` / `Rechazado` / `Error`), read from the `tbai_syncinvoice` table. It is *not* a contract field: it is injected server-side per row by `TbaiSyncStatusInjector.inject(dataArr)`. Until this ticket that injector was only called from `SalesInvoiceHeaderHandler.afterHandle()`; **ETP-5087 also wires it into `PurchaseInvoiceHeaderHandler.afterHandle()`** (`com.etendoerp.go`, a companion change shipped with this same ticket), so purchase rows now carry the same multi-state value sales rows always did.
2. **Fallback — `row.tbaiIssent`.** The invoice's own AD boolean `EM_Tbai_Issent` ("Enviada a Batuz"), which flips to true once the invoice has been submitted. It is used **only** when `tbaiSyncEstado` is absent — i.e. the invoice has no row in `tbai_syncinvoice` yet, or the backend predates the injector wiring above. It proves the invoice was *submitted*, nothing about the result.

**Why this order, and why the fallback stays.** The two fields answer different questions. Only `tbaiSyncEstado` can report a **rejection**; reading the boolean first would let a rejected invoice render as a cheerful "Enviada". Keeping the boolean as a fallback means the column never defaults to a fabricated status when the real state is simply not there yet. Belt and braces: the column never lies by default, and never hides a rejection.

`tbaiIssent` reaches the browser through `artifacts/purchase-invoice/decisions.json`:

```json
"tbaiIssent": { "visibility": "readOnly", "form": false }
```

`readOnly` (rather than `discarded`/`system`) is what puts the field in `frontendContract.entities.header.fields`, so NEO serialises it on every list row; `form: false` keeps it out of the detail form, where it would be a meaningless non-editable checkbox. Same pattern as `adOrgId` and the `etvfac*` fields on this window. **Do not re-discard it** — it is the fallback leg of the expression above.

NEO may serialise an AD boolean either as JSON `true`/`false` or as the raw AD character flag `'Y'`/`'N'`. `'N'` is a truthy JS string, so the fallback uses the exported `isSent()` helper from `shared/sifSending.js` (`value === true || value === 'Y'`) rather than a bare truthy test — the same idiom `SifTab`, `SifDataTabs` and `useSifFieldPatcher` already use.

**Badge states.** `Recibido` / `Rechazado` / `Error` come straight from the injected state and map to the existing `fiscalMonitor.tbai.status.*` badges. The fallback adds `Enviada` (`fiscalMonitor.tbai.status.Enviada`, tone `success`) for a submitted-but-unresolved invoice and `Pendiente` for everything else. `Enviada` deliberately does **not** reuse `Recibido`: that badge means "accepted by the Diputación", a stronger claim than a send flag can back.

**Sales-invoice already worked this way and was NOT modified.** `sales-invoice`'s TBAI column has always read `row.tbaiSyncEstado` (with no fallback, since its injector has always run). The defect was never in the sales window nor in the field choice — it was that the purchase-side handler had no producer for `tbaiSyncEstado`, and the frontend's `?? 'Pendiente'` turned that absence into a hardcoded "Pendiente" on **every** purchase row: a silent false datum instead of a visible failure. This section replaces that behaviour; see the ETP-5087 entry in `docs/feedback.md` for the generalizable lesson.

**Column visibility is global and synchronous — the documented trade-off.** Both fiscal columns derive from one memo:
...
`profile`/`territory` come from `useFiscalConfig(selectedOrg.id, apiBaseUrl)` — the org selected in the top-nav. The Bizkaia-only rule is **not** restated in the component: `getInvoiceFiscalTargets` is the single place that decides `showTbai` is only ever true for a purchase invoice whose territory is `BIZKAIA`.

The consequence, accepted deliberately: a page that mixes legal entities from *different* territories shows or hides the Batuz column according to the top-nav org, not per row. This is **exactly** the trade-off the SII column has always made, and it is why the two columns are now symmetric.

**Why the per-row mechanism was removed.** An earlier revision resolved each row's own org (`resolveInvoiceOrgId`) and batched a `{ profile, territory }` fetch per distinct org through a `useOrgFiscalConfigs` hook, then gated the column and each cell on the result. That asynchrony produced three consecutive production regressions in a row:

1. the Batuz column never appeared, because column visibility waited on a fetch that had not resolved on first render;
2. the **SII** column disappeared too, because an intermediate version folded both columns into the same per-row reduction — SII had never needed it;
3. the hook returned the same mutated `Map` reference on every render, so callers memoizing off it never recomputed and the column stayed frozen at its empty-cache result until an unrelated re-render (switching list tabs) shook it loose.
...
| Step | Selector | Purchase key | Sales key |
|------|----------|--------------|-----------|
| Confirmation body | `getSifBodyKey(specName, { sendSii, sendTbai })` | `sendToSifBodyTbaiPurchase` / `sendToSifBodyBothPurchase` | `sendToSifBodyTbai` / `sendToSifBodyBoth` |
| Result — TBAI OK | `getSifTbaiSuccessKey(specName)` | `sendToSifSuccessTbaiPurchase` | `sendToSifSuccessTbai` |
| Result — TBAI failed | `getSifTbaiErrorKey(specName)` | `sendToSifErrorTbaiPurchase` | `sendToSifErrorTbai` |

`SifSendingModal.jsx` resolves the two result keys from the `specName` prop it already receives; it never hardcodes a TBAI key. The SII keys (`sendToSifBodySii`, `sendToSifSuccessSii`, `sendToSifErrorSii`) never mention TBAI and are shared by both directions, so they needed no variant, and there is no combined SII+TBAI *result* key — the results phase renders one line per target.

The result half was the later fix: the confirmation copy shipped purchase-aware while `SifSendingModal.jsx` still rendered `ui('sendToSifSuccessTbai')` unconditionally, so a purchase invoice the user had just confirmed sending to **Batuz** reported back "Enviado a TicketBAI correctamente." — the popup contradicted itself between its own two steps. Any future copy added to this flow must be routed through a `sifSending.js` selector for the same reason.
...
- **QA-flagged, non-blocking gap:** the `_tbaiStatus` list column and the `invoicePreview.fiscalStatus.tbaiPurchase` detail-panel label (both from "List Batuz column — real sync state with a send-flag fallback, synchronous global visibility (ETP-5087)" above) have Vitest/component coverage (`PurchaseInvoiceHeaderTable.vitest.jsx`, `InvoicePreview.vitest.jsx`) but no Playwright coverage — `sif-buttons-fiscal-config.spec.js`'s 2 mocked cases exercise the `Enviar a SIF` button/popup wording only, not the list-grid badge column or the preview InfoRow label. Worth a follow-up spec if this window's E2E suite is revisited.
...
16. Open "Nuevo pago" for an invoice whose business partner has a preferred `pOFinancialAccount` supporting the invoice's method and confirm both **Método de pago** and **Cuenta** preselect to those values; change **Método de pago** to one the preferred account does not support and confirm **Cuenta** re-filters to only the accounts supporting the new method, reselecting per the BP-preferred → `default`-flagged → first-supporting priority. Select a saldo a favor/crédito line that covers 100% of the outstanding amount (leaving Importe at 0,00 €) and confirm **Confirmar** is enabled, not disabled. From the invoice list, click the payment cell in the **Saldo pendiente** column to open the history popup, register a payment, and confirm both the popup's own **Saldo pendiente** stat and the list's **Saldo pendiente** column update immediately without a page reload.
...
18. (ETP-5087, needs the companion `com.etendoerp.go` change deployed) With a Bizkaia TBAI org selected in the top nav, open the purchase-invoice list and confirm the **Estado Batuz** column appears next to **Estado SII**. Confirm an invoice already accepted by the Diputación reads **Recibido** (not "Enviada") and a rejected one reads **Rechazado** — a rejection must never render as "Enviada" or "Pendiente". Then confirm an invoice submitted but with no row yet in `tbai_syncinvoice` falls back to **Enviada**, and a never-submitted one to **Pendiente**. Switch the top-nav org to a Gipuzkoa/Álava TBAI org and confirm the column disappears while **Estado SII** stays.
...
`useFiscalConfig`/`getPendingSifTargets`, and also covering TBAI when applicable — TBAI only
applies to purchase invoices on a Bizkaia territory, see "TBAI territory gating for purchase
invoices (Batuz) — ETP-5087" above — see the acceptance test above). An earlier iteration of the ETP-4888 point 2 fix additionally added a
...
**document** currency instead of the currency the price list is expressed in. On a purchase invoice switched to
...
`window.selectorPriceCurrency` was absent from `artifacts/purchase-invoice/decisions.json`. The resolution
...
`window.selectorPriceCurrency: "org"` in `artifacts/purchase-invoice/decisions.json`, which makes
...
1. Open a draft purchase invoice whose header currency equals the organization currency (EUR) and open the
...
- `artifacts/purchase-invoice/decisions.json` — `window.selectorPriceCurrency: "org"`.
- `artifacts/purchase-invoice/contract.json` — `selectorPriceCurrency` carried into the window block.
