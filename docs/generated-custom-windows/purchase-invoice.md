...
**Sales-invoice already worked this way and was NOT modified by ETP-5087.** `sales-invoice`'s TBAI column has always read the same primary source with no fallback, since its injector has always run. The defect was never in the sales window nor in the field choice — it was that the purchase-side handler had no producer for the injected field, and the frontend's `?? 'Pendiente'` turned that absence into a hardcoded "Pendiente" on **every** purchase row: a silent false datum instead of a visible failure. This section replaces that behaviour; see the ETP-5087 entry in `docs/feedback.md` for the generalizable lesson.
...
- **QA-flagged, non-blocking gap:** the `eTGOTbaiStatus` list column and the `invoicePreview.fiscalStatus.tbaiPurchase` detail-panel label (both from "List Batuz column — real sync state with a send-flag fallback, synchronous global visibility (ETP-5087)" above) have Vitest/component coverage (`PurchaseInvoiceHeaderTable.vitest.jsx`, `InvoicePreview.vitest.jsx`) but no Playwright coverage — `sif-buttons-fiscal-config.spec.js`'s 2 mocked cases exercise the `Enviar a SIF` button/popup wording only, not the list-grid badge column or the preview InfoRow label. Worth a follow-up spec if this window's E2E suite is revisited.
