...
- **Record — header form:** the form is flat and borderless (`noHeaderBorder: true`, `whiteFormBackground: true`), matching the Assets window style. Three principal fields appear side-by-side: **Name** (editable), **Movement Date** (editable), **Document No.** (read-only, auto-generated, in the `principal` section). The former "Others" tab no longer exists: `description` is set to `form: false` so it is hidden from the header form, and `documentNo` was moved into the principal section. All three header fields become fully read-only once the document is processed.
  - **(ETP-5491) Document No. source and timing.** The number comes from the `DocumentNo_M_Movement` table sequence, the same as in Classic. `M_Movement` has no `C_DocType_ID` column, so the "Inventory Move" document type (`DocBaseType = 'MMM'`) and its sequence are not used; core falls back to the table sequence. `GoodsMovementsHeaderHandler` (`com.etendoerp.go`) assigns it **once, on create** (the CRUD `POST` without a record id). A real value sent in the body wins; null, blank and `<...>` placeholders are replaced. Actions on an existing movement — **Process** (`POST /{id}/action/processNow`), Post/Unpost — never consume a number, so processing no longer skips one (previously every POST, including the Process action, consumed a number). The new-record form's `<...>` preview (from `/defaults`) reads the same sequence without consuming it, so the previewed number matches the one assigned on save.
...
- **Record — product picker (shared drawer):** the `product` field declares `lookup: true`, `lookupDrawer: "product-stock"`, `lookupTitle: "Product"`, and `onSelectMappings: [{ from: "_aux._LOC", to: "storageBin", labelFrom: ["warehouse", "warehouse$_identifier", "storageBin"] }]`. This opens `ProductStockSearchDrawer.jsx`, the window-agnostic product+stock picker registered in `tools/app-shell/src/components/contract-ui/lookupDrawers.js` under the key `product-stock` (shared with `internal-consumption`). The drawer groups results by product, offers warehouse-filter pills, and expands each product to show its per-locator stock rows (warehouse name — every warehouse has exactly one locator, so the locator code adds no information). It is backed by the AD "Product Complete" selector (reference 800011 / `M_Product_Stock_V`). On select, it auto-fills `storageBin` (source bin) from `_aux._LOC`, labeled by warehouse name.
  - **(ETP-5282) Zero-stock rows are excluded server-side, not merely hidden client-side.** `M_PRODUCT_STOCK_V` is a `UNION ALL` of real `M_Storage_Detail` rows and a synthetic "every active product, `qtyonhand = 0`, no locator" row (`stocked = 'N'`) so every product is pickable somewhere in the app — that view itself is unchanged, other consumers (e.g. receipts) still need the synthetic row. For the `movementLine` source specifically, `GoodsMovementProductSelectorPolicy` (`com.etendoerp.go`) now appends `stocked = true` to the selector's HQL filter, so the synthetic zero-stock row never reaches this drawer at all — a product with no real stock anywhere no longer appears in the picker, and a product with stock at one locator and none at another shows only the stocked locator's row (row-level exclusion, the product itself is never dropped). Previously the on-hand quantity shown for a zero-stock row was documented as "informational only" because the picker let the user select it regardless; that is no longer possible for this window — every row the drawer shows is now backed by real, positive stock. The same policy also excludes Service-type products (ETP-4606, unchanged) and applies identically to `inventoryLine` (Physical Inventory) and `internalConsumptionLine` (Internal Consumption).
  - The lookup drawer registry (`lookupDrawers.js`) is a generic, opt-in mechanism — any window can reuse `product-stock` or register a custom drawer by adding an entry there. (`goods-movements-product` and `internal-consumption-product` remain as deprecated aliases of the same shared component.)
...
13. **(ETP-5282)** Open the product picker for a product with NO stock at any warehouse/locator. Confirm it does not appear in the picker at all (previously it appeared with `0 ud`). Then open the picker for a product that has stock at one locator and none at another; confirm only the stocked locator's row appears, not a zero-stock row for the same product, and that the product itself is still pickable (not excluded entirely).
...
- (ETP-5282) `com.etendoerp.go` — `src/com/etendoerp/go/schemaforge/selector/policy/GoodsMovementProductSelectorPolicy.java` and its JUnit suite `src-test/src/com/etendoerp/go/schemaforge/selector/policy/GoodsMovementProductSelectorPolicyTest.java` cover the `stocked = true` HQL filter this drawer's selector now carries for the `movementLine` source (also `inventoryLine` and `internalConsumptionLine`).
