...
- **No send-email ("Enviar") action (ETP-4717, superseding ETP-4718):** this window has no Enviar/send-email affordance on any surface (grid row-hover, form/preview action bar). It briefly existed under ETP-4718 but was removed after QA found every send attempt failed with "Unknown email contract" — see the ETP-4717 entry in the Gap assessment section below for the root cause and removal detail.
...
- **ETP-4717 — Send-email action removed (superseding ETP-4718's wiring):** `artifacts/return-to-vendor-shipment/decisions.json` sets `window.sendDocument: { enabled: false }` again, and the generated `ReturnToVendorShipmentPage.jsx` passes `sendDocument={{"enabled":false}}` (instead of the bare, auto-enabled `sendDocument` ETP-4718 left behind) to both the `ListView` and `DetailView` usages. `windows/custom/return-to-vendor-shipment/index.jsx` no longer passes an `emailAction` prop to `ReturnWindowShell.jsx` (nor the `useReturnToVendorPdf`/`useMenuLabel`/`tMenu` wiring that only existed to build it). `ReturnToVendorShipmentPreview.jsx` no longer computes `isSendable` or passes `onEmail` to `buildReturnPreviewContent.jsx`. Root cause: the frontend's `${windowName}-send` contract-name convention (`return-to-vendor-shipment-send`) never matched the backend's registered `ReturnToVendorSendEmailContract.NAME` (`return-to-vendor-send`) — ETP-4718's claim that the contract "already existed" was unverified. QA (Emilio Polliotti) asked for removal, not a name fix; the backend contract itself is untouched by this PR. See the ETP-4717 bullet in Gap assessment for full detail.
