...
9. **ETP-5124:** Open the row-preview modal (right-panel eye icon) for any receipt — confirm the left panel shows the Etendo-generated PDF (spinner while rendering, then the document), not an upload drop-zone. Confirm no attachment upload/drop-zone control appears in the preview's left panel. If the customer brought their own return document, confirm it is attached instead from the **Attachments** tab (step 8), not from the preview panel. Confirm the "Facturado" percentage row and badge in the right panel status card, plus the "Invoice Status" grid column, reflect the real invoiced percentage for that record (not always 0%).
...
## Preview left panel shows the Etendo-generated PDF again — ETP-5124

QA (Isaías) rejected the delivery: the row-preview's left panel must show the
Etendo-generated PDF, matching AC#5 ("El panel de preview muestra el PDF generado a la
izquierda"). The PM (Valeria, with Emilio Polliotti) confirmed after analysis that this was
the originally intended requirement and that the ETP-4408 change (see the struck-through
paragraph above) had it backwards: *"Debe ser un documento generado por Etendo que tiene el
fin de entregar como constancia al cliente de que su producto fue devuelto en
conformidad."* If the customer brings their own commercial return document, per the ticket
it does **not** replace the Etendo-generated one — it is attached from the Adjuntos section
for traceability (AC#8), which this window already exposes via the generic Attachments tab
(restored by commit `63204c7f4`, unaffected by this change).

`tools/app-shell/src/windows/custom/return-material-receipt/ReturnMaterialReceiptPreview.jsx`
reverts to the pre-ETP-4408 wiring: `leftPanel` renders `PreviewPdfPanel` fed by
`useReturnReceiptPdf`'s `pdfUrl`/`pdfLoading`/`pdfError`, and the single-file
`attachmentConfig` (`tableName: 'M_InOut', useMainAttachment: true, storeCondition: true,
autoFetch: false`) that used to hold the customer's uploaded receipt is removed — that slot
is no longer surfaced anywhere in this window's UI (customer documents now live exclusively
in the generic multi-file Attachments tab).

**Deliberate scope decision — no PDF caching added.** Unlike the sibling
`return-to-vendor-shipment`/`goods-shipment` (which pass `pdfCacheConfig` /
`attachmentConfig` with `sourceBlob`/`recordUpdated` to auto-store the rendered PDF as the
record's marked `M_InOut` attachment and read it back on the next open, per
`docs/document-printables.md` §"The PDF cache"), this window's fix does **not** enable that
layer. Any record whose preview was opened under the ETP-4408 behavior (2026-07-06 onward)
may already have a customer-supplied file marked as the main `M_InOut` attachment; turning on
the read-side cache now would risk serving that stale customer document back as if it were a
cached Etendo PDF — reintroducing the exact bug this ticket fixes, just through the cache
path instead of the left panel wiring. `docs/document-printables.md`'s opt-out list is updated
accordingly (D18). A follow-up ticket should verify (via a DB query for `M_InOut` records with
a marked attachment predating a real Etendo-rendered re-render) whether any such stale
attachments exist in production before enabling caching for this window.

`return-to-vendor-shipment` is explicitly **out of scope** for ETP-5124's left-panel fix — its
preview already shows the system-generated PDF by default and was never touched by ETP-4408's
customer-upload override; the PM has not been asked to change it and nothing here does.
