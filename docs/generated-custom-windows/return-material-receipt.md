...
- Line interaction: the child table and form expose line number, product, movement quantity, UOM, order quantity, and sales-order-line context. **Line No. is now also a grid column, not just a form field (ETP-5316):** `lineNo` (`M_InOutLine.Line`) was already visible in the line form but was `grid: false`; it is now the leading grid column (`gridOrder: 1`, read-only), so a raw-line-number backend error surfaced during "Confirmar" (e.g. "línea 10") can be matched back to a row. `product`/`movementQuantity`/`orderQuantity` shifted from `gridOrder: 1/2/3` to `2/3/4`.
