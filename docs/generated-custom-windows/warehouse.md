...
**Triggered from both save paths (ETP-5199).** `createDefaultStorageBin` is wired as `WarehouseWindow`'s `onAfterCreate` prop (`windows/custom/warehouse/index.jsx`), which the shared `DetailView.jsx` component invokes after a successful create. It now reliably fires regardless of which action creates the record: the toolbar Save button (`handlePostSaveNavigation` in `tools/app-shell/src/components/contract-ui/saveActions.jsx`) AND the in-app "Guardar y salir" unsaved-changes-navigation-guard prompt (`buildUnsavedChangesSaver`, same file, registered via `useUnsavedChangesGuard` in `DetailView.jsx`). Before ETP-5199, "Guardar y salir" called only `hook.handleSave` and skipped the post-save hook entirely — creating a new warehouse and confirming "Guardar y salir" (instead of clicking the toolbar Save button) silently skipped default-storage-bin creation. Verified live via the storage-bin list endpoint: before the fix, querying the new warehouse's storage bins returned `totalRows: 0`; after the fix, the same query returns `totalRows: 1` with the expected `<searchKey>-0-0-0` default bin. See `docs/generated-custom-windows/user.md`'s "Save-lifecycle hook" section for the full shared mechanism (this generic extension point on `DetailView.jsx` is not window-specific — Users' role-assignment `onAfterExistingSave` hook is the other consumer and was the one where this gap was originally found) and its "Error-propagation contract asymmetry" note for how a throwing `onAfterCreate` is handled differently on each save path.

**This window's own error handling makes the generic fallback toast a non-issue here.** `handleAfterCreate` (above) already wraps its `createDefaultStorageBin` call in a try/catch and shows its own `toast.warning` — on both save paths equally, since both now funnel through the same `onAfterCreate` prop. Because of that, the generic `savedButFollowUpActionFailed` toast the "Guardar y salir" path's `buildUnsavedChangesSaver` would otherwise show on a throwing hook never actually surfaces for Warehouse: `handleAfterCreate` never rejects, it always resolves (having already reported the failure itself). That generic toast exists as a safety net for a future window whose `onAfterCreate`/`onAfterExistingSave` does not catch its own errors.
...
- `tools/app-shell/src/components/contract-ui/saveActions.jsx` (`runAfterSaveHook`, `buildUnsavedChangesSaver`) and `tools/app-shell/src/components/contract-ui/DetailView.jsx` — the shared, window-agnostic mechanism that invokes `onAfterCreate` from BOTH the toolbar Save button and the "Guardar y salir" unsaved-changes-guard path (ETP-5199). Covered in `tools/app-shell/src/components/contract-ui/__tests__/DetailView.saveActions.vitest.jsx` (including a dedicated "Warehouse window" throwing-`onAfterCreate` case) and the wiring regression guard `tools/app-shell/src/components/contract-ui/__tests__/DetailView.unsavedChangesSaver.test.js`. See `docs/generated-custom-windows/user.md`'s "Save-lifecycle hook" and "Error-propagation contract asymmetry" sections for the full mechanism, shared with Users' role-assignment hook.
...
3a. (ETP-5199) Repeat step 3 twice for two DIFFERENT new warehouses: once saving via the toolbar Save button, once saving by making an edit and confirming "Guardar y salir" from the in-app unsaved-changes navigation prompt instead. Confirm BOTH paths create the default storage bin (query `/storageBin?parentId=<warehouseId>` and confirm `totalRows: 1` in both cases) — before this fix, only the toolbar Save button did.
