...
- **Main list total**: the `totalAmortization` column uses `type: 'amount'`, which is what gives it the currency symbol AND the column footer total — `DataTable` totals every `amount` column unless one opts out. Its `summable: true` is the explicit form of that default and is not what produces the total; a column that must be formatted as money but NOT added up declares `summable: false` instead (ETP-5245 — see `docs/decisions-reference.md` §"Amount columns: formatting vs. totals").
