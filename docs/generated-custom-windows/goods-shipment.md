...
- Independent accounting date (ETP-4531): `accountingDate` (`DateAcct`) is now a full editable header field (own default `@#Date@`, principal section, locked once `Posted='Y'` — deliberately `Posted` rather than this window's dominant `Processed='Y'` pattern, since `Processed='Y'` alone would also lock the date during a failed-posting-attempt state where it may still need correcting before retry) — previously hidden (`visibility: system`). `M_InOut.MovementDate` (shared table with Goods Receipt) carries `AD_Column.AD_Callout_ID = org.openbravo.erpCommon.ad_callouts.SL_InOut_AccountingDate`, which auto-fills `dateAcct` from `movementDate`. `com.etendoerp.go`'s `NeoCalloutService` executes this same classic callout server-side. The auto-fill side effect is neutralized on every path: `GoodsShipmentHeaderHandler#afterCallout` (new override; the `NeoHandler` default was a no-op) strips a callout-driven `accountingDate` update when the trigger field isn't `accountingDate` itself (mirrors the existing `blockCalloutCurrencyUpdate`/ETP-4029 pattern), and `NeoDefaultsCascadeHelper#processCalloutForField` applies the same strip generically to the GET `/defaults` (new-record bootstrap) and POST create cascades, which the `afterCallout` hook alone does not reach. `M_InOut.DateAcct` itself carries no callout (verified against the DB), so accountingDate has no self-triggered side effect to check here (unlike the invoice windows' `SE_Invoice_TaxDate`).
