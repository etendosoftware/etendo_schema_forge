...
- Independent accounting date (ETP-4531): `accountingDate` (`DateAcct`) is now a full editable header field (own default `@#Date@`, principal section, locked once `Posted='Y'`) — previously hidden (`visibility: system`). `M_InOut.MovementDate` (shared table with Goods Receipt) carries `AD_Column.AD_Callout_ID = org.openbravo.erpCommon.ad_callouts.SL_InOut_AccountingDate`, which auto-fills `dateAcct` from `movementDate`. `com.etendoerp.go`'s `NeoCalloutService` executes this same classic callout server-side, so changing `movementDate` today still overwrites `accountingDate` behind the scenes — a `GoodsShipmentHeaderHandler#afterCallout` guard (currently the `NeoHandler` default no-op; mirrors the existing `blockCalloutCurrencyUpdate`/ETP-4029 pattern) is required in `com.etendoerp.go` to strip that side effect; tracked as a backend follow-up.
