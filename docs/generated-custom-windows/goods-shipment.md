...
- Batch invoicing from the list is constrained by current UI logic: only completed shipments with `invoiceStatus < 100` are counted as invoiceable, and all selected invoiceable shipments must belong to the same business partner before `Create Invoice` is enabled. Since ETP-4028, batch invoicing also requires all selected shipments to share the same currency (`currencyCheck` guard) — mixed-currency selections disable `Create Invoice` with an explanatory tooltip. **Since the bulk unification (2026-09):** the batch flow opens the SAME shared `CreateInvoiceConfirmModal` ("Gestionar documentos") the single-shipment flow uses, with a required **Tarifa** (price list) selector — a full price-list picker was previously unimplemented for this flow; it is now the same one. This trades away per-line selection and quantity editing: the batch invoice now always takes the **full pending quantity** of every line in the selected shipments (the backend's pending-quantity cap still prevents double-invoicing), and there is no more "existing draft invoice" warning banner in this flow. Partial-quantity batch invoicing remains available only from the single-shipment flow described above.
- **The card shows a real quote, not a "N envíos" count (2026-09).** `BulkInvoiceFromShipment.jsx` computes it per line — for a line with a linked sales order line, the **order line's own `unitPrice`** (`GET /sales-order/lines/{orderLineId}`), never the chosen Tarifa; for a line with no linked order, the **selected Tarifa's price** for that product (`GET /sales-invoice/lines/selectors/M_Product_ID?priceList=…`, the same `ProductPriceSelectorPolicy` selector the manual line-entry callout cascade uses). This mirrors Core's own `UpdatePricesAndAmounts.java` (`createlinesfromprocess` package) exactly: `exec()` copies the order line's `unitPrice` whenever the line has (or is) a related order line, and only falls back to the invoice's price list otherwise — verified directly in that file, not inferred. The number shown is therefore not an estimate: it is the real amount the created invoice will carry (rounding aside), and only the order-unlinked portion of it reacts to changing the Tarifa dropdown. A line whose price cannot be resolved from either source contributes nothing to the total — the same "left blank for the user to fill in" gap the single-shipment flow already has for a priceless product. Falls back to the plain "N envíos" count only while the quote hasn't resolved yet (no Tarifa chosen, or the per-line fetches are still in flight).
...
- **Resolved (2026-09):** the top-bar single-shipment gate (`GoodsShipmentActions.jsx`) already used `invoiceStatus >= 100`. The bulk selection-bar gate (`BulkInvoiceFromShipment.jsx`) used to check `r.completelyInvoiced !== true` instead — a no-op, since `completelyInvoiced` (`Iscompletelyinvoiced`) is renamed to `invoiced` with `grid: false, form: false` in `decisions.json` and never reaches a grid row at all. Fixed to `parseFloat(r.invoiceStatus ?? 0) < 100`, the same field and same threshold the single-record gate already used — the two are now consistent.
...
8. In the batch invoice modal ("Gestionar documentos"), confirm the card shows "N envíos" until a Tarifa is chosen, then updates to a real currency amount once resolved; confirm a **Tarifa** must be selected before "Crear →" enables, and that the subtitle shows the total pending units summed across the selected shipments. There is no per-line selection or quantity editing in this flow (available only from a single shipment's own "Crear Factura").
9. Select shipments whose lines all come from a linked sales order and confirm the quote does **not** change when a different Tarifa is picked (the order's own price wins). Select (or mix in) shipments with lines that have no linked order and confirm that portion of the quote **does** react to the Tarifa change. After creating the invoice, confirm its actual total matches the quote shown in the modal.
...
- `artifacts/goods-shipment/custom/BulkInvoiceFromShipment.jsx` implements the toolbar button, the invoiceable/same-BP/same-currency guards, and the pending-units total (summed via one `pendingInvoiceLines` call per selected shipment); it opens the shared `CreateInvoiceConfirmModal` (imported from `@/components/contract-ui/CreateInvoiceConfirmModal`, not a bespoke modal) and shows the result through the shared `ConfirmResultModal` (`docs.type: 'facturaVenta'`, route `/sales-invoice/{id}`), which already badges confirmed-vs-draft off `documentStatus` (ETP-5381) — the component itself no longer hand-rolls a result toast.
...
- `artifacts/goods-shipment/custom/__tests__/BulkInvoiceFromShipment.test.js` provides source-shape coverage for the bulk invoice component: invoiceable-row filtering (`invoiceStatus`, not the dead `completelyInvoiced` field), same-BP/same-currency guards, that it opens `CreateInvoiceConfirmModal` (not a bespoke modal) with a computed `cardAmountLabel`/`pendingQtyTotal`/`onPriceListChange`, the `createDraftInvoice` POST body shape (`shipmentIds` + `priceListId`, no per-line `lines`), that it uses `useApiFetch` (never a bare `fetch`), and that it ends with `clearSelection()` → `refresh()` from the result modal's `onClose`. A dedicated `quote — real price per line, not an estimate` block asserts: it fetches shipment lines and `pendingInvoiceLines` (both needed for the quote, not just the subtitle), the order-line price fetch (`sales-order/lines/{id}`) is **not** keyed on `selectedPriceListId`, the Tarifa-price fetch filters to lines with no linked order line, the `price = detail.salesOrderLine ? orderLinePrices[…] : tariffPrices[…]` precedence, and that the total is formatted with `formatCurrency`. `CreateInvoiceConfirmModal.vitest.jsx` covers the new `onPriceListChange` prop itself (fires on initial resolve, fires again on user change, safe no-op when omitted).
...
### Guard P2 — REMOVED (superseded, 2026-09)

An earlier revision of ETP-5381 added `CreateDraftInvoiceHandler.assertShipmentsHavePending`,
which threw a dedicated `AlreadyInvoicedException` (HTTP 409, literal "This shipment has
already been fully invoiced.") when every selected shipment had nothing pending. **Both the
guard and the exception class were removed two days later** ("Remove the last create-time
invoice restriction" / "Drop the guards the core already enforces once confirmed"): once the
invoice is confirmed on creation, `C_Invoice_Post` raises `m_inoutline.isinvoiced`/`qtyinvoiced`
in the same transaction, and the pre-existing pending-quantity logic already rejects a second
invoice on its own — the guard was a second opinion on top of a check that now works. **There
is no 409 in this flow any more; do not reintroduce one.** A shipment with nothing pending
simply contributes no lines to the batch selection (or is filtered out client-side by
`invoiceStatus < 100` before the request is even sent).
...
2. Trigger `Create Invoice` again on that same shipment and verify the pending-units subtitle
   reads 0/absent and the confirm button stays disabled (client-side), or — if attempted
   directly against the API — that Core's own pending-quantity check rejects it (there is no
   longer a dedicated 409 for this case).
3. Repeat from the batch flow with a selection mixing one fully-invoiced shipment
   and one with pending lines, and verify the run is **not** blocked — the already-invoiced
   shipment simply contributes no lines.
...
- `CreateDraftInvoiceHandlerTest.java` and `NeoInvoiceSupportTest.java` were extended for the create-and-confirm flow (including `computePendingQtyPerLineOrThrow`'s throwing behavior, used where a DB failure must surface as an error rather than degrade into "nothing pending").
