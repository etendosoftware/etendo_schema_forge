...
- **2026-09-22 (ETP-5444) — `M_Warehouse.C_Location_ID` maps to property `locationAddress`** (`getLocationAddress()`/`setLocationAddress(Location)`), same convention as `OrganizationInformation.C_Location_ID`. Confirmed via `javap -p` on the compiled `org.openbravo.model.common.enterprise.Warehouse` class (no `.java` source is checked into any repo here — it's `src-gen`, generated by the build). `Location` itself exposes `addressLine1`/`addressLine2`/`cityName`/`postalCode`/`postalAdd`/`country`/`region`/`regionName`/`city` — confirmed the same way.
- **2026-09-22 (ETP-5444) — GOClient's sampledata ships TWO `M_WAREHOUSE` rows** ("Almacen Principal", `C_LOCATION_ID=A72B4E2D0E5A4369B45219D3D83E4FDC`, and "Almacen Secundario", `C_LOCATION_ID=98F19CA220F648B7A9D47FEB41D90DA8`) but only the first reaches a real tenant — `OnboardingDemoMasterData.WAREHOUSE_IDS` includes the secondary warehouse's id, and `DemoMasterDataFilter` (inside `OnboardingDatasetNormalizer`'s `RowExclusionFilter`) drops it at import time (see gap N4 in `onboarding-and-datafixes-map.md`). **Apply:** any onboarding step that needs "the" default warehouse should resolve it via `WarehouseLookupHelper.findFirstActiveWarehouse`, not by assuming a fixed sampledata id — the secondary one never survives to a tenant anyway, but relying on hardcoded warehouse ids from the XML is fragile regardless.
- **2026-09-22 (ETP-5444) — the default warehouse's address and the org's fiscal address are two INDEPENDENT `C_Location` rows, populated by two different onboarding steps at two different times.** The warehouse's location comes from the GOClient sampledata import (step 1, `importOnboardingDataset`) — a fixed placeholder address baked into `C_LOCATION.xml`. The fiscal address (`AD_ORGINFO.C_Location_ID`) is created later by `OnboardingOrgInfoService#ensureOrgInfoLocation` from the wizard's optional "Address" field. Nothing links them by construction — see gap I2. **Apply:** don't assume any two `C_Location` FKs on different entities point at "the same address" just because they're both created during onboarding; check which step created each one.
