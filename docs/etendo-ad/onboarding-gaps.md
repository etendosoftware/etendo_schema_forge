...
| B2 | Tenant subscription | A tenant marked productive before `ETGO_SUBSCRIPTION` existed has only the legacy `AD_Preference` `ETGO_TenantPlan='productive'` marker and NO subscription row, so every consumer of the new subscription model reads a paying tenant back as unsubscribed | Corrective-only data-fix (`R37-tenant-subscription-backfill`) — inserts one open row on the grandfathered `legacy-productive` plan per productive tenant, copying the Stripe ids and the charged price id from its `ETGO_CHECKOUT_REQUEST` when one exists, seeding `STATUS` from `ETGO_SubscriptionStatus` (`PAST_DUE`→`past_due`, `EXPIRED`/`NONE`→`canceled`, else `active`) and `CURRENT_PERIOD_END` from `ETGO_SubscriptionDueAt` so no tenant's effective access improves, and retiring the tenant's `ETGO_TenantPlan` preference in the same transaction; aborts loudly (FAILED, so the tenant retries) if the plan row is missing. CUT deliberately NOT bumped — once the `ETGO_TenantPlan` write path is retired a new tenant never gets the marker, so `@check` is a 0-row skip for it. Must run **after** the module deploy; design: `com.etendoerp.go/docs/plans/2026-09-18-etp-5046-plan-and-subscription-design.md` §7–§8 | ETP-5046 |
