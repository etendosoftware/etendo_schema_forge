...
| B2 | Tenant subscription | A tenant marked productive before `ETGO_SUBSCRIPTION` existed has only the legacy `AD_Preference` `ETGO_TenantPlan='productive'` marker and NO subscription row, so every consumer of the new subscription model reads a paying tenant back as unsubscribed | Corrective-only data-fix (`R37-tenant-subscription-backfill`) — inserts one open row on the grandfathered `legacy-productive` plan per productive tenant, copying Stripe ids from its `ETGO_CHECKOUT_REQUEST` when one exists; aborts loudly (FAILED, so the tenant retries) if the plan row is missing. CUT deliberately NOT bumped — once the `ETGO_TenantPlan` write path is retired a new tenant never gets the marker, so `@check` is a 0-row skip for it | ETP-5046 |
