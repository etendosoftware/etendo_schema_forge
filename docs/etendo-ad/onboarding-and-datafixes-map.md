...
| **A4** | `A_Amortization` (`AD_Table_id 800060`) inactive on `c_acctschema_table` — amortization documents can't post; missed by TC-39's checklist — ETP-4452 | `C_ACCTSCHEMA_TABLE.xml` row `DAE3C688574C4919B889DA7EFAD6CC5C`'s `ISACTIVE` flipped `N`→`Y`; `ONBOARDING_PROVISIONED_THROUGH` bumped to `2026-07-08T10:00:00Z` | `R13-amortization-table-active` — single guarded `UPDATE`, self-scoping via the same `65000000`-marker `@check` as R12 (naturally excludes QA Testing/TaxesOrg's unrelated US-chart schemas); live-validated on acreedortest, acreetest2, empresa (`APPLIED (1 row)` each → re-run `SKIPPED_NOT_NEEDED`); GOClient already `Y` live → `SKIPPED_NOT_NEEDED` on first run |
