/**
 * Behavioral test for the window-level read-only gate on the DetailView footer
 * Save action (ETP-5075).
 *
 * A GO view-only window (decisions.json → window.readOnly, threaded to the
 * frontend as api.window.readOnly === true, or the runtime `window` prop
 * override) must never expose the footer Save action, mirroring the existing
 * Delete-button gate covered by DetailView.windowReadOnly.vitest.jsx. DetailView
 * now ANDs `!windowReadOnly` into both `saveActionsFirst` and
 * `!saveActionsFirst` render branches around `renderSaveActions(saveActionParams)`.
 *
...
function makeHook(data, { isDirtyHeader = true } = {}) {
  return {
    loading: false,
    items: [],
    selected: data,
...
let currentHook = makeHook({ id: '123', documentNo: 'CR-001', status: 'DR', documentStatus: 'DR', processed: false });
...
describe('DetailView — window.readOnly footer Save gate (ETP-5075)', () => {
  beforeEach(() => {
    navigateMock.mockClear();
    currentHook = makeHook({ id: '123', documentNo: 'CR-001', status: 'DR', documentStatus: 'DR', processed: false }, { isDirtyHeader: true });
  });

  it('hides the footer Save button on an existing dirty record when api.window.readOnly is true', () => {
    renderDetailView({ api: { window: { readOnly: true } } });
    expect(screen.queryByTestId('action-save')).toBeNull();
  });

  it('shows the footer Save button on an existing dirty record when api.window.readOnly is absent (regression)', () => {
    renderDetailView({ api: {} });
    expect(screen.getByTestId('action-save')).toBeTruthy();
  });

  // Mirrors the runtime per-tier override case already covered for Delete
  // (ETP-4520): the `window` prop, distinct from the static api.window.readOnly.
  it('hides the footer Save button on an existing dirty record when window.readOnly is true (prop override)', () => {
    renderDetailView({ api: {}, window: { readOnly: true } });
    expect(screen.queryByTestId('action-save')).toBeNull();
  });
