...
describe('DetailView line sidebar save/delete flow (classic layout)', () => {
  ...
  it('saves the edited line via PATCH then refetches and updates the child', async () => {
    ...
    // ETP-4576 — the refetch is a safe GET: it authenticates with the `__Host-`
    // session cookie (hence `credentials`), carries no bearer token, and gets no
    // CSRF proof. Same intent as before — the second call is a bare GET with only
    // headers, no method and no body — expressed in the cookie-session contract.
