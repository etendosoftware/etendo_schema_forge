    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-foreground/40 p-4 md:p-8" data-testid="upgrade-page-shell">
      ...
      <div className="grid gap-5 md:grid-cols-3">
        <PlanCard
          ...
          // The catalog is the truth: it carries the amount derived from the provider price the
          // checkout will actually charge. The server-owned billing offer is the fallback while
          // the catalog lookup is in flight or has failed, and when neither can answer the card
          // says so rather than quoting a number nobody verified — advertising one price and
          // charging another is the worst outcome this page can produce.
          price={plans.length > 0
            ? formatPlanPrice(ui, plans.find(plan => plan.planKey === selectedPlanKey) || plans[0])
            : (formatOfferPrice(billingOffer, getStoredLocale())
