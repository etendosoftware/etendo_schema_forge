    ...
    <div className="space-y-6 p-6" data-testid="ApiKeysPage">
      ...
      <Card data-testid="Card__5d273f"><CardContent className="p-0" data-testid="CardContent__5d273f">
        {loading ? <div className="flex justify-center py-12 text-muted-foreground"><RefreshCw className="mr-2 h-5 w-5 animate-spin" data-testid="RefreshCw__5d273f" />{ui('apiKeysLoading')}</div> : keys.length === 0 ? <div className="py-16 text-center"><KeyRound
          ...
          onClick={() => runAction(key, `Rotate "${key.name}"?`, 'The old secret will stop working immediately.', 'Rotate', async () => { const result = await rotateApiKey(apiFetch, key.id); setReveal({ clientId: result.clientId, clientSecret: result.clientSecret }); })}
          data-testid="DropdownMenuItem__5d273f"><KeyRound className="mr-2 h-4 w-4" data-testid="KeyRound__5d273f" />Rotate secret</DropdownMenuItem><DropdownMenuItem
          onClick={() => runAction(key, `Revoke tokens for "${key.name}"?`, 'All active access tokens will be invalidated.', 'Revoke tokens', async () => { await revokeApiKeyTokens(apiFetch, key.id); toast.success('Tokens revoked'); })}
          data-testid="DropdownMenuItem__5d273f"><Ban className="mr-2 h-4 w-4" data-testid="Ban__5d273f" />Revoke tokens</DropdownMenuItem><DropdownMenuSeparator data-testid="DropdownMenuSeparator__5d273f" /><DropdownMenuItem
          className="text-destructive"
          onClick={() => runAction(key, `Delete "${key.name}"?`, 'This permanently removes the credential.', 'Delete', async () => { await deleteApiKey(apiFetch, key.id); toast.success('API key deleted'); })}
          data-testid="DropdownMenuItem__5d273f"><Trash2 className="mr-2 h-4 w-4" data-testid="Trash2__5d273f" />Delete</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell></TableRow>)}</TableBody></Table>}
      </CardContent></Card>
      <Dialog
        open={dialog.open}
        onOpenChange={(open) => setDialog((state) => ({ ...state, open }))}
        data-testid="Dialog__5d273f"><DialogContent data-testid="DialogContent__5d273f"><form onSubmit={submit}><DialogHeader data-testid="DialogHeader__5d273f"><DialogTitle data-testid="DialogTitle__5d273f">{dialog.key ? 'Edit public API key' : 'Create public API key'}</DialogTitle><DialogDescription data-testid="DialogDescription__5d273f">{dialog.key ? 'Change the display name or active state.' : 'Choose a name and the public capabilities this key may request.'}</DialogDescription></DialogHeader><div className="grid gap-4 py-4"><div className="grid gap-2"><Label htmlFor="api-key-name" data-testid="Label__5d273f">Name</Label><Input
        id="api-key-name"
        value={name}
        onChange={(event) => setName(event.target.value)}
        maxLength={120}
        placeholder="e.g. Nest integration"
        required
        data-testid="Input__5d273f" /></div>{!dialog.key && <><div className="grid gap-2"><Label data-testid="Label__5d273f">Allowed capabilities</Label>{CAPABILITIES.map(([capability, description]) => <label className="flex items-start gap-2 rounded-md border p-3 text-sm" key={capability}><input type="checkbox" checked={selected.includes(capability)} onChange={() => setSelected((values) => values.includes(capability) ? values.filter((value) => value !== capability) : [...values, capability])} /><span><span className="font-mono">{capability}</span><span className="block text-xs text-muted-foreground">{description}</span></span></label>)}</div><div className="rounded-md border border-primary/20 bg-primary/5 p-3 text-sm"><p className="font-medium">Connect it from Scalar</p><p className="mt-1 text-muted-foreground">After creation, copy the generated credential and paste it into Scalar's <span className="font-medium">bearer</span> authorization field. Use <code className="rounded bg-muted px-1 font-mono text-xs">clientId:clientSecret</code>; do not add the word <code className="rounded bg-muted px-1 font-mono text-xs">Bearer</code>.</p><a className="mt-2 inline-flex items-center gap-1 text-primary underline underline-offset-4" href={SCALAR_DOCS_URL} target="_blank" rel="noreferrer">Open Scalar docs <ExternalLink className="h-3.5 w-3.5" data-testid="ExternalLink__5d273f" /></a></div></>}<div className="flex items-center justify-between"><Label htmlFor="api-key-active" data-testid="Label__5d273f">Active</Label><Switch
        id="api-key-active"
        checked={active}
        ...
        data-testid="Button__5d273f">Cancel</Button><Button type="submit" disabled={submitting} data-testid="Button__5d273f">{submitting ? 'Saving…' : dialog.key ? 'Save changes' : 'Create key'}</Button></DialogFooter></form></DialogContent></Dialog>
