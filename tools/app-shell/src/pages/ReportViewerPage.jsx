...
function SelectorPopup({ open, onClose, onSelect, selector, title, extraParams = {} }) {
  ...
  const fetchPage = useCallback((q, off, append) => {
    ...
    return fetch(`${ETENDO_BASE}/sws/report-selectors/${selector}?${params}`, { headers: readCredentialHeaders() })
      .then(r => r.json())
      .then(data => {
...
function SearchInput({ selector, value, displayValue, onChange, multi, minLength = 2, fullWidth = false, hasError = false, token, label, selectedOrgId, roleOrgIds, selectedWarehouseId, extraParams = {} }) {
  ...
  const fetchOptions = useCallback((q) => {
    ...
    fetch(`${ETENDO_BASE}/sws/report-selectors/${selector}?${params.toString()}`, { headers: readCredentialHeaders() })
      .then(r => r.json())
...
function PopupMultiSelector({ selector, label, onChange, value = '', displayValue = '' }) {
  ...
  useEffect(() => {
    if (!open) return;
    const t = setTimeout(() => {
      fetch(`${ETENDO_BASE}/sws/report-selectors/${selector}?q=${encodeURIComponent(query)}`, { headers: readCredentialHeaders() })
        .then(r => r.json())
    ...
    Promise.all(
      autoParams.map(p => {
        ...
        const orgParam = (p.selector === 'currency' && selectedOrgId)
          ...
          : '';
        return fetch(`${ETENDO_BASE}/sws/report-selectors/${p.selector}?q=${orgParam}`, { headers: readCredentialHeaders() })
          .then(r => r.json())
