// ETP-5030 — selected-row shading for the Verifactu monitor table.
//
...
//
// This is a separate file from VerifactuMonitorSection.vitest.jsx on purpose:
// that suite mocks `useFmSelection` with a permanently-empty Set, which
// structurally cannot express "tick a checkbox and watch the row change". The
// stub below keeps that file's FmPrimitives factory shape but gives
// `useFmSelection` a real, stateful implementation. Nothing is added to
// FmPrimitives.jsx: the five suites that mock it with exhaustive hand-written
// factories stay untouched.
import { render, screen, fireEvent, within, waitFor } from '@testing-library/react';
import { describe, it, expect, vi } from 'vitest';

// `verifactuSendingStatus: 'AC'` maps to 'accepted', which is what the default
// "correct" tab lists.
const ROWS = [
  { id: 'vf-1', invoice: 'inv-1', 'invoice$documentNo': 'V-001', verifactuSendingStatus: 'AC', cSV: 'CSV-1' },
  { id: 'vf-2', invoice: 'inv-2', 'invoice$documentNo': 'V-002', verifactuSendingStatus: 'AC', cSV: 'CSV-2' },
];

// Rows arrive through the normal list fetch, not through the `mockRows` prop:
// the section's "reset on tab change" effect runs right after the list effect on
// mount and clears `rows`, so the synchronous `mockRows` branch never survives
// to render (pre-existing behaviour, unrelated to ETP-5030 — the async fetch
// path resolves after that reset).
//
...
const baseProps = {
  ...
  kpis: { verifactu: { accepted: 2, partiallyAccepted: 0, rejected: 0, invalid: 0 } },
};
...
describe('VerifactuMonitorSection — ETP-5030 selected-row shading', () => {
  async function renderRows() {
    const view = render(<VerifactuMonitorSection {...baseProps} />);
  ...
  it('marks ONLY the ticked row as selected and leaves the others unmarked', async () => {
    ...
    expect(classesOf(rowOf('V-001'))).toEqual([]);
    expect(classesOf(rowOf('V-002'))).toEqual([]);

    fireEvent.click(checkboxIn(rowOf('V-001')));

    expect(classesOf(rowOf('V-001'))).toEqual(['fm-row--selected']);
  ...
  it('keeps the marker on the row while it is hovered', async () => {
    ...
    fireEvent.click(checkboxIn(rowOf('V-001')));
    fireEvent.mouseOver(rowOf('V-001'));
    fireEvent.mouseEnter(rowOf('V-001'));
  ...
  it('select-all marks every row, and clearing it unmarks every row', async () => {
    ...
    expect(classesOf(rowOf('V-001'))).toEqual(['fm-row--selected']);
    expect(classesOf(rowOf('V-002'))).toEqual(['fm-row--selected']);
