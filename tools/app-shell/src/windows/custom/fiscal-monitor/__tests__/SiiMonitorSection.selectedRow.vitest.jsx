// ETP-5030 — selected-row shading for the SII monitor table.
//
...
//
// This is a separate file from SiiMonitorSection.vitest.jsx on purpose: that
...
const ROWS = [
  { id: 'sii-1', documentNo: 'F-001', invoiceDate: '2026-05-10', aeatsiiEstado: 'CO', grandTotalAmount: 100 },
  { id: 'sii-2', documentNo: 'F-002', invoiceDate: '2026-05-11', aeatsiiEstado: 'CO', grandTotalAmount: 200 },
];
...
// path resolves after that reset, which is why the sibling suites use it too).
//
...
const baseProps = {
  ...
  parentId: 'parent-1',
  kpis: { sii: { issued: 2, received: 0, issuedPrevious: 0, receivedPrevious: 0 } },
};
...
describe('SiiMonitorSection — ETP-5030 selected-row shading', () => {
  async function renderRows() {
    const view = render(<SiiMonitorSection {...baseProps} />);
  ...
  it('marks ONLY the ticked row as selected and leaves the others unmarked', async () => {
    ...
    expect(classesOf(rowOf('F-001'))).toEqual([]);
    expect(classesOf(rowOf('F-002'))).toEqual([]);

    fireEvent.click(checkboxIn(rowOf('F-001')));

    expect(classesOf(rowOf('F-001'))).toEqual(['fm-row--selected']);
  ...
  it('keeps the marker on the row while it is hovered', async () => {
    ...
    fireEvent.click(checkboxIn(rowOf('F-001')));
    fireEvent.mouseOver(rowOf('F-001'));
    fireEvent.mouseEnter(rowOf('F-001'));
  ...
  it('select-all marks every row, and clearing it unmarks every row', async () => {
    ...
    expect(classesOf(rowOf('F-001'))).toEqual(['fm-row--selected']);
    expect(classesOf(rowOf('F-002'))).toEqual(['fm-row--selected']);
