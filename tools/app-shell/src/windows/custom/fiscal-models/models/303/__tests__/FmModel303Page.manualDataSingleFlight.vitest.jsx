...
describe('FmModel303Page — single-flight manualData autosave (ETP-5255)', () => {
  ...
  // ETP-4576 — this case USED to assert the opposite: that a queued save is dropped when
  // `token` goes falsy mid-flight. That eligibility gate is exactly what the cookie scheme
  // forbids — the client holds no token there, so `!!token` is permanently false and manual
  // data would never save at all, for anybody. The guard now keys on `apiBaseUrl` alone (see
  // the sibling case below, which still covers the not-eligible path), and a replay issued
  // after the session really ended comes back 401, which `apiFetch` turns into a logout —
  // a better outcome than silently losing the edit. So the replay MUST fire here.
  it('replays a queued save after the client token disappears, because the session is not in the client', async () => {
    const server = installServer();
