...
export default function RelatedDocuments({ recordId, data, token, apiBaseUrl, docsRefreshSignal }) {
    ...
    const originInvoicesPromise = originInvoiceIds.length > 0
      ? Promise.all(
          originInvoiceIds.map(id =>
            fetchById('purchase-invoice', 'header', id, token, apiBaseUrl).catch(() => null)
          )
        )
      : Promise.resolve([]);
  ...
  }, [recordId, apSubtype, data?.salesOrder, data?.linkedReceipts, data?.originInvoices, data?.originInvoice, token, apiBaseUrl, refreshKey, docsRefreshSignal]);
