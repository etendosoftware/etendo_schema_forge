 ...
 */
function ActiveStatusToggle({ data, recordId, apiBaseUrl, onRefresh }) {
  const ui = useUI();
...
export default function UserWindow(props) {
  ...
  useEffect(() => {
    ...
    // ETP-4576 — `token` was part of this second gate, and under a cookie session it
    // is structurally undefined, so the effect returned early forever: the saved
    // template roles were never seeded into the selection, so a user who already had
    // roles opened with an empty control and "Guardar" looked clean while the real
