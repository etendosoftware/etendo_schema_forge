...
describe('useYearCloseStatus', () => {
  it('resolves to true when the end-year-close endpoint returns at least one row', async () => {
    ...
    const { result } = renderHook(() => useYearCloseStatus('year1', 'https://api.test/end-year-close'));
  ...
  it('stays undefined and never fetches when yearId is absent', () => {
    global.fetch = vi.fn();
    const { result } = renderHook(() => useYearCloseStatus(undefined, 'https://api.test/end-year-close'));
  ...
  it('re-fetches when yearId changes', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ yearId }) => useYearCloseStatus(yearId, 'https://api.test/end-year-close'),
