import { useState } from 'react';
import { Loader2, Landmark } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from '@/components/ui/dialog';
import { useUI } from '@/i18n';

/**
 * Renders the two native surfaces of the bank connect flow driven by {@code useBankConnectionFlow}:
 * a non-dismissable "waiting for bank authentication" overlay while the Salt Edge popup is open,
 * and the bank-account selection modal shown when the connection returns more than one account.
 *
 * @param {{ flow: ReturnType<typeof import('@/hooks/useBankConnectionFlow').useBankConnectionFlow> }} props
 */
export function BankConnectionFlowUI({ flow }) {
  const ui = useUI();
  const { connecting, selection, confirmSelection, cancelSelection } = flow;

  return (
    <>
      <Dialog open={connecting} data-testid="Dialog__bankconnflow">
        <DialogContent
          className="max-w-sm bg-card"
          onPointerDownOutside={(e) => e.preventDefault()}
          onEscapeKeyDown={(e) => e.preventDefault()}
          data-testid="bank-connection-connecting-overlay"
        >
          <DialogHeader data-testid="DialogHeader__bankconnflow">
            <DialogTitle className="flex items-center justify-center gap-2 text-center" data-testid="DialogTitle__bankconnflow">
              <Loader2 className="h-5 w-5 animate-spin text-[var(--status-info-fg)]" data-testid="Loader2__bankconnflow" />
              {ui('financeAccountsBankConnectionConnecting')}
            </DialogTitle>
          </DialogHeader>
          <p className="text-center text-xs text-[hsl(var(--muted-foreground))]">{ui('financeAccountsBankConnectionConnectingHint')}</p>
        </DialogContent>
      </Dialog>
      <BankConnectionAccountSelectModal
        selection={selection}
        onConfirm={confirmSelection}
        onCancel={cancelSelection}
        data-testid="BankConnectionAccountSelectModal__5f0f32" />
    </>
  );
}

function BankConnectionAccountSelectModal({ selection, onConfirm, onCancel }) {
  const ui = useUI();
  const [selected, setSelected] = useState(null);

  const open = !!selection;
  const accounts = selection?.accounts ?? [];
  const providerName = selection?.providerName;
  const providerLogoUrl = selection?.providerLogoUrl;

  return (
    <Dialog
      open={open}
      onOpenChange={(value) => { if (!value) onCancel?.(); }}
      data-testid="Dialog__bankconnselect">
      <DialogContent className="bg-card" data-testid="bank-connection-account-select-modal">
        <DialogHeader data-testid="DialogHeader__bankconnselect">
          <DialogTitle className="flex items-center gap-2" data-testid="DialogTitle__bankconnselect">
            {providerLogoUrl ? (
              <img
                src={providerLogoUrl}
                alt=""
                className="h-6 w-6 rounded border border-[hsl(var(--border-subtle))] bg-card object-contain p-0.5"
              />
            ) : null}
            <span>
              {providerName
                ? ui('financeAccountsBankConnectionSelectTitleBank', { bank: providerName })
                : ui('financeAccountsBankConnectionSelectTitle')}
            </span>
          </DialogTitle>
        </DialogHeader>
        <p className="text-sm text-[hsl(var(--muted-foreground))]">{ui('financeAccountsBankConnectionSelectHint')}</p>
        <div className="mt-2 flex flex-col gap-2">
          {accounts.map((acc) => {
            const isSelected = selected === acc.saltEdgeAccountId;
            return (
              <button
                type="button"
                key={acc.saltEdgeAccountId}
                onClick={() => setSelected(acc.saltEdgeAccountId)}
                data-testid={`bank-connection-account-option-${acc.saltEdgeAccountId}`}
                className={`flex items-center gap-3 rounded-lg border p-3 text-left transition-colors ${
                  isSelected
                    ? 'border-[var(--status-info-fg)] bg-[var(--status-info-bg)]'
                    : 'border-[hsl(var(--border-subtle))] hover:bg-[hsl(var(--muted))]'
                }`}
              >
                <Landmark
                  className="h-5 w-5 flex-none text-[hsl(var(--muted-foreground))]"
                  data-testid="Landmark__5f0f32" />
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-medium text-[hsl(var(--foreground))]">
                    {acc.name || acc.iban || acc.saltEdgeAccountId}
                  </span>
                  <span className="block truncate text-xs text-[hsl(var(--muted-foreground))]">
                    {[acc.iban, acc.currency].filter(Boolean).join(' · ')}
                  </span>
                </span>
              </button>
            );
          })}
        </div>
        <div className="mt-4 flex justify-end gap-2">
          <button
            type="button"
            onClick={onCancel}
            data-testid="bank-connection-account-select-cancel"
            className="rounded-lg border border-[hsl(var(--border-subtle))] px-4 py-2 text-sm font-medium text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))]"
          >
            {ui('cancel')}
          </button>
          <button
            type="button"
            disabled={!selected}
            onClick={() => onConfirm?.(selected)}
            data-testid="bank-connection-account-select-confirm"
            className="rounded-lg bg-[hsl(var(--foreground))] px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-[hsl(var(--accent-highlight))] hover:text-[hsl(var(--accent-highlight-foreground))] disabled:bg-[hsl(var(--border-control))] disabled:text-primary-foreground disabled:hover:bg-[hsl(var(--border-control))] disabled:hover:text-primary-foreground"
          >
            {ui('financeAccountsBankConnectionSelectConfirm')}
          </button>
        </div>
      </DialogContent>
    </Dialog>
  );
}
