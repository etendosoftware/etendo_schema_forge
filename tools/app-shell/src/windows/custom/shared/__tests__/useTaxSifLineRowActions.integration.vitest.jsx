...
describe('useTaxSifLineRowActions + TaxSifModal (real, end-to-end) — compound/summary tax resolution', () => {
  ...
  function CompoundHarness() {
    const { cellBadges, modal } = useTaxSifLineRowActions({
      // ETP-4576 — no `token` option. The hook's requests take their credential from
      // the active session scheme, so the local TOKEN constant this used to pass was
      // removed with the sweep; the epic added these cases against it in parallel,
      // which git merged without a conflict because neither side touched the other's
      // line.
      apiBaseUrl: API_BASE_URL, enabled: true, recordId: RECORD_ID, windowCategory: 'sales',
    });
