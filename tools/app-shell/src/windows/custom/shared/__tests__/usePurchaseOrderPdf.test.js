...
describe('usePurchaseOrderPdf', () => {
  ...
  it('exports usePurchaseOrderPdf as a named export', () => {
    assert.match(src, /export function usePurchaseOrderPdf/);
  });
  ...
  it('accepts orderId, apiBaseUrl, token and optional currencyData parameters', () => {
    ...
    assert.match(src, /usePurchaseOrderPdf\(orderId,\s*apiBaseUrl,\s*token,\s*currencyData/);
  });
  ...
  it('delegates data building to shared buildOrderData', () => {
    ...
    assert.match(src, /'purchase-order'/, 'passes purchase-order spec to buildOrderData');
  });

  // ── API endpoints ─────────────────────────────────────────────────────────

  it('fetches header from the purchase-order header endpoint', () => {
    assert.match(sharedSrc, /\$\{spec\}\/header\/\$\{orderId\}/);
  });

  it('fetches lines from the purchase-order lines endpoint with parentId', () => {
  ...
  it('fetches session to resolve yourCompanyDocumentImageId', () => {
  ...
  it('uses purchaseOrderPdfTitle i18n key for the document title label', () => {
    assert.match(src, /purchaseOrderPdfTitle/);
  });

  it('uses purchaseOrderPdfSection i18n key for the document section label', () => {
    assert.match(src, /purchaseOrderPdfSection/);
  });
