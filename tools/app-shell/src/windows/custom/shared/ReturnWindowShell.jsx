  ...
  if (recordId) {
    return (
      <PageComponent
        ...
        token={token}
        autoSaveOnBlur={true}
        /* ETP-5408: no `hasExternalPrimaryAction` any more (ETP-4933 set it here while both
           return windows rendered their own Confirm in topbarRight). Both now pass a
           `draftMode` (see their index.jsx), so the generic Save Draft (outline) + Confirm
           pair renders and Save is already the secondary look — the flag is only read by
           renderNewRecordSaveActions, which a draftMode window never reaches. */
        {...pageProps}
