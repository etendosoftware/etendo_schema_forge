...
export function useMainAttachment({
  ...
  const refresh = useCallback(async () => {
    ...
    setIsBusy(true);
    try {
      const main = await fetchMainAttachment({ tableName, recordId: documentId, apiBaseUrl });
      if (!main) {
        revokeUrl();
        setStoredFile(null);
  ...
  const uploadAndMark = useCallback(async (blob, fileName, mimeType) => {
    ...
    const created = await uploadAndMarkMainAttachment({
      tableName, recordId: documentId, file: blob, fileName, apiBaseUrl,
    });
  ...
  const storeUrl = useCallback(async (url, fileName) => {
    ...
    setStoreFailed(false);
    try {
      const res = await fetch(url, { headers: readCredentialHeaders(), credentials: 'include' });
  ...
  const markExisting = useCallback(async (attachmentId, fileName, mimeType) => {
    ...
    const ok = await markAttachmentAsMain({ attachmentId, isMain: true, apiBaseUrl });
    if (!ok) return false;
    const objectUrl = await fetchAttachmentBlobUrl({ attachmentId, apiBaseUrl });
    if (objectUrl) {
      applyAttachment(attachmentId, fileName, mimeType, objectUrl);
    }
    notifyAttachmentsChanged({ tableName, recordId: documentId, source: sourceRef.current });
    return true;
  }, [active, apiBaseUrl, applyAttachment, tableName, documentId]);

  const deleteFile = useCallback(async () => {
    if (!active || !storedFile?.attachmentId) return;
    const result = await deleteAttachment({ attachmentId: storedFile.attachmentId, apiBaseUrl });
    if (!result?.ok) return;
    revokeUrl();
    setStoredFile(null);
    setStoredFileIsStale(false);
