...
export default function PaymentHeaderTableBase({ dir, specName, data, onNavigate, apiBaseUrl, ...props }) {
  ...
  const handleReactivate = async (row) => {
    const url = `${apiBaseUrl}/${entity}/${row.id}/action/etprReactivatePayment`;
    try {
      ...
      if (res.ok) {
        toast.success(ui(dir === 'in' ? 'cobroReactivadoOk' : 'pagoReactivadoOk'));
        setReactivateRow(null);
        window.dispatchEvent(new CustomEvent('neo:processSuccess'));
      } else {
        const body = await res.json().catch(() => ({}));
        toast.error(body.message || body.error || ui(dir === 'in' ? 'cobroReactivadoError' : 'pagoReactivadoError'));
      }
    } catch {
      toast.error(ui(dir === 'in' ? 'cobroReactivadoError' : 'pagoReactivadoError'));
    }
  };

  const menuActions = useCallback(({ row }) => {
    if (!DEPOSITED_STATUSES.has(row.status || '')) return [];
    return [{
