...
import { writeHeaders } from './financialAccountHttp';
 ...
 */
export async function postStatementFile(action, { accountId, fileName, contentBase64 }) {
...
export function useStatementFileRequest(action) {
  ...
  const run = useCallback(async (payload) => {
    ...
    setError(null);
    try {
      return await postStatementFile(action, payload);
    } catch (err) {
      setError(err);
      throw err;
    } finally {
