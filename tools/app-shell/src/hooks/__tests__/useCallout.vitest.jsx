...
describe('useCallout', () => {
  ...
  // ── peekBusinessPartnerCallout (ETP-5024 follow-up) ────────────────────────
  //
  // Isolated, non-debounced one-shot check used by useEntity.js's
  // handleProcess/handleSaveAndProcess right before a Complete/Confirm
  // request. Must fire immediately (no 300ms wait), must NEVER touch
  // calloutResult/calloutLoading (that state feeds DetailView's auto-apply
  // effect), and must be best-effort (never throw, resolve to null on any
  // failure).
  describe('peekBusinessPartnerCallout (ETP-5024)', () => {
    beforeEach(() => {
      // Real timers: this function does not debounce, so fake timers only get
      // in the way of awaiting its promise.
      vi.useRealTimers();
      globalThis.fetch = vi.fn();
    });
    ...
    it('fires immediately — no debounce wait needed', async () => {
      globalThis.fetch.mockResolvedValue({
        ok: true,
        json: async () => ({ updates: {}, combos: {}, messages: [] }),
      });
      ...
      await act(async () => {
        await result.current.peekBusinessPartnerCallout('BP001', { id: '1' });
      });

      // No vi.advanceTimersByTime needed — the fetch already happened.
      expect(globalThis.fetch).toHaveBeenCalledTimes(1);
      expect(globalThis.fetch).toHaveBeenCalledWith(
        'http://localhost/api/header/callout',
    ...
    it('resolves to the classified condition on a matching response', async () => {
      globalThis.fetch.mockResolvedValue({
        ok: true,
        json: async () => ({
          updates: {}, combos: {},
      ...
      let condition;
      await act(async () => {
        condition = await result.current.peekBusinessPartnerCallout('BP001', { id: '1' });
      });
    ...
    it('resolves to null on a non-matching response', async () => {
      globalThis.fetch.mockResolvedValue({
        ok: true,
        json: async () => ({ updates: {}, combos: {}, messages: [{ type: 'INFO', text: 'Unrelated message' }] }),
      });
    ...
    it('resolves to null (never throws) on a non-ok response', async () => {
      globalThis.fetch.mockResolvedValue({ ok: false, status: 500 });
    ...
    it('resolves to null without calling fetch when value is empty', async () => {
      const { result } = renderHook(() => useCallout('header', opts));

      let condition;
      await act(async () => {
        condition = await result.current.peekBusinessPartnerCallout('', { id: '1' });
      });
    ...
    it('never calls setCalloutResult — calloutResult/calloutLoading stay untouched', async () => {
      globalThis.fetch.mockResolvedValue({
        ok: true,
        json: async () => ({
          updates: { priceList: 150 }, combos: { paymentTerm: [{ id: 'PT1' }] },
          messages: [{ type: 'ERROR', text: 'Business Partner credit limit exceeded' }],
        }),
      });
      ...
      // A response carrying updates/combos/a matching message would, through
      // executeCallout, populate calloutResult (and DetailView's auto-apply
      // effect would then apply `updates`/`combos` to the form). The peek must
      // leave this hook's own state completely untouched, proving the isolation
      // from DetailView's calloutResult-driven auto-apply effect.
      expect(result.current.calloutResult).toBeNull();
      expect(result.current.calloutLoading).toBe(false);
    });
  });
