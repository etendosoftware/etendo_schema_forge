...
export async function resolveSavedRecordAfterSave(saved, {
    ...
    try {
        const refetchRes = await fetch(`${apiBaseUrl}/${entity}/${saved.id}`, { headers, credentials: 'include' });
...
export function useEntity(entity, childEntity, {
    ...
    const refresh = useCallback(() => {
        ...
        fetch(`${apiBaseUrl}/${entity}?${queryParams.toString()}`, { headers, credentials: 'include' })
            .then(res => {
                if (res.status === 401) {
                    logout();
                    throw new Error('401');
    ...
    const fetchChildDefaults = useCallback(async (parentId) => {
        ...
        try {
            const res = await fetch(`${apiBaseUrl}/${childEntity}/defaults?parentId=${parentId}`, { headers, credentials: 'include' });
            if (!res.ok) throw new Error(`${res.status}`);
            const data = await res.json();
        ...
        // NOTE: `headers` is intentionally omitted — buildHeaders() returns a
    ...
    const fetchById = useCallback((id) => {
        ...
        setLoading(true);
        fetch(`${apiBaseUrl}/${entity}/${id}`, { headers, credentials: 'include' })
            .then(res => {
                if (!res.ok) throw new Error(`${res.status}`);
                return res.json();
            })
    ...
    }, [apiBaseUrl, entity, fetchChildren, neutralizePendingDefaults]);
    ...
    const handleNew = useCallback(async () => {
        ...
        try {
            const res = await fetch(`${apiBaseUrl}/${entity}/defaults`, {
              headers,
              credentials: 'include',
    ...
    }, [childEntity, apiBaseUrl, selected, headers, fetchChildren, ui, refreshHeaderTotals, applyExemptionCauseSignals]);
    ...
    const handleSaveAndProcess = useCallback(async (draftModeConfig) => {
        ...
        try {
            const updatedRes = await fetch(`${apiBaseUrl}/${entity}/${saved.id}`, { method: 'GET', headers, credentials: 'include' });
            if (updatedRes.ok) {
