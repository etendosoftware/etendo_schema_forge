...
/**
 * ETP-4393 — verify every new i18n key shipped by the OAuth2 token validity
 * period feature (the MCP connector consent selector) exists in BOTH locale
 * files (en_US.json and es_ES.json) under genericLabels with a non-empty
 * translation. Prevents missing-key fallbacks from regressing into hardcoded
 * English in the AuthorizePage consent view.
 */
...
const ETP_4393_KEYS = [
  ...
  'oauthValidity1Month',
  'oauthValidityNever',
  'oauthTokenExpiresLabel',
];

describe('ETP-4393 — i18n key parity', () => {
  let enUS;
  let esES;
  ...
  for (const key of ETP_4393_KEYS) {
