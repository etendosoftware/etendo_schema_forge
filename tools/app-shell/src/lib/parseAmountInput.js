/**
 * CANONICAL parser for an amount string whose separators are AMBIGUOUS — a bank-statement cell, a
 * spreadsheet export, or anything typed into a field that has no live masking to show the user how
 * its input is being read. Originally `windows/custom/financial-account/statementAmount.js`
 * (ETP-4954); promoted to `lib/` under ETP-5107's reopened round when the payment/movement helpers
 * hit the exact same ambiguity and a third hand-rolled rule was about to be written for it.
 * `statementAmount.js` now re-exports from here, so the bank-statement callers are unchanged.
 *
 * ## How this differs from the other two canonical number helpers — read before changing it
 *
 * `formatCurrency` and `parseLocaleNumber` are CONFIG-DRIVEN: they read the instance's separators
 * from `getCurrencyFormatConfig()`. This parser deliberately is NOT, and must not become so. It
 * decides by the STRUCTURE of the string (digit counts), because its inputs do not come from one
 * declared convention — see "Why the digit count" below. Making it read the configured separator
 * reintroduces the silent 1000x corruption ETP-4954 closed.
 *
 * Use `parseLocaleNumber` for text typed into a MASKED field (the component already guarantees a
 * clean, unambiguous value and the user sees the interpretation live). Use this one when the
 * string may carry grouping and nothing has disambiguated it yet.
 *
 * ## The rule
 *
...
/**
 * An editable form field's view of an amount: a finite number, with blank and unparseable both
 * collapsing to 0. Used where there is no room to show a per-field error, so an unreadable amount
 * reads as "no amount here" and the surrounding required/`> 0` gate blocks the save — which is
 * the desired outcome, since the alternative (`parseFloat`'s numeric prefix) silently invents a
 * plausible-looking wrong amount from malformed input.
 */
export function parseAmountOrZero(v) {
  const n = parseAmountInput(v);
  return Number.isFinite(n) ? n : 0;
}

/** True when the value holds something that is not a number. A blank value counts as valid. */
export function isInvalidAmountInput(raw) {
  return Number.isNaN(parseAmountInput(raw));
}
