const BACKEND_ERROR_MAP = {
  ...
  // ETP-5111, the unified delete rule for the Financial Account window. Every guard below is a
  // refusal the user can now REACH, because the actions that used to hide themselves are offered
  // on every row; `movementActionEligibility.js` pre-checks the movement ones client-side against
  // these very keys, so the kebab and the bulk/REST/MCP paths read one sentence, not two.
  //
  // Split by direction where the owning document differs (pago is payment-out, cobro payment-in):
  // telling a receipt's owner to go to "the payment" sends them to the wrong window. Split by
  // action for the same reason — reactivating is not deleting, and they have different
  // consequences — so each pair names the action the user was actually attempting.
  ...
  // Pre-checks the APRM_FIN_FINACC_TRAN_CHECK_TRG trigger rather than letting it fire: a JDBC
  // error escapes the handler's OBException branch and arrives as an opaque HTTP 500.
  'The movement is already processed or posted. Select it on its own to delete it.':
    'backendError.movementProcessedNotDeletable',
  // The two statement refusals, in the order a user meets them: processed, then — once reactivated
  // — still carrying a reconciled line that deleting the statement would take with it.
 ...
 *
 * The backend space-joins every applicable REASON_* sentence behind the prefix above, so one
 * account could report four near-identical reasons the user cannot act on differently — whichever
 * they resolved, it still would not delete. Matching only the prefix also retires a nine-literal
 * wire contract: a tenth blocker on the Java side needs no change here. The individual reasons
 * stay in the 409 body and the server log; only the toast is generic.
 */
