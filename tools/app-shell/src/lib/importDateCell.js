/**
 * The one date parser for IMPORT cells, shared by every window whose import reads a date.
 *
 * It started as `financial-account/statementDate.js` and was promoted here unchanged when the
 * Product import needed the same four shapes for the costing starting date (ETP-5350). Nothing
 * about it was ever bank-specific: an operator types `dd/MM/yyyy` into a product file exactly as
 * they do into a statement file, and `parseXlsx` emits `dd-MM-yyyy` for a real Excel date cell
 * whatever the window. A second copy would have been a second set of edge cases to get wrong.
 *
 * Lives in its own module, apart from the pipelines that use it, so a row validator can reach it
 * without an import cycle (a pipeline imports its descriptor, so the descriptor cannot import the
 * pipeline). Same split, and same reason, as `financial-account/statementAmount.js`.
 */
 ...
 */
export function isInvalidImportDate(raw) {
  const s = String(raw ?? '').trim();
  return s !== '' && normalizeImportDate(s) === null;
}
