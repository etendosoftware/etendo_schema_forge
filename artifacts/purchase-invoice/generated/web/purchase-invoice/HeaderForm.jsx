...
const fields = [
  ...
  { key: 'transactionDocument', column: 'C_DocTypeTarget_ID', type: 'selector', label: 'Transaction Document', required: true, section: 'principal', reference: 'DocumentType', inputMode: 'selector', readOnlyLogic: (record) => record['processed'] === true },
  { key: 'documentNo', column: 'DocumentNo', type: 'text', label: 'Document No.', required: true, readOnly: true, section: 'principal', maxLength: 30, readOnlyLogic: (record) => record['processed'] === true },
  { key: 'orderReference', column: 'POReference', type: 'text', label: 'Supplier Reference', section: 'principal', maxLength: 20, readOnlyLogic: (record) => record['aeatsiiIssent'] === true && record['salesTransaction'] !== true },
  { key: 'invoiceDate', column: 'DateInvoiced', type: 'date', label: 'Invoice Date', required: true, section: 'principal', readOnlyLogic: (record) => record['posted'] === true || (record['processed'] === true && (record['documentStatus'] !== 'VO')) || ((record['aeatsiiEstado'] === 'CO' || record['aeatsiiEstado'] === 'AE') && record['salesTransaction'] === true) },
