...
describe('purchase-invoice contract integrity (ETP-3778 SIF regressions)', () => {
  ...
  // ETP-4933's regen (commit aface9264) surfaced a readOnlyLogic on orderReference that
  // did not exist in the previous contract: the AD now locks POReference once the
  // invoice has been submitted to the AEAT SII (`@EM_Aeatsii_Issent@='Y' & @IsSOTrx@='N'`).
  // Confirmed correct with Gremiger (author of the original ETP-3778 guards): this does
  // NOT conflict with ETP-3778 (docs/generated-custom-windows/purchase-invoice.md:222,
  // "correct the supplier reference on a completed invoice without reactivating it") —
  // the AD rule says nothing about `processed`/`documentStatus`, so a completed-but-not-
  // yet-declared invoice stays fully editable. Only SII submission locks it. Do NOT
  // "fix" this back to `readOnlyLogic === undefined`, and do NOT let it drift into an
  // unconditional read-only (e.g. by adding a `processed` clause) — either would silently
  // reopen the case ETP-3778 fixed, or silently break the new SII-lock requirement.
