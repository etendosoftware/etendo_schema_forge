...
// from the purchase-order line's own `unitPrice` field, mirroring the pattern
// already used by sales-invoice/custom/ImportFromOrderModal.jsx against the
// twin sales-order/lines endpoint. Today this file never sets either field on
// the mapped line, so ImportLinesModal's docTotal (sum of _lineNetAmount over
...
describe('ImportFromPurchaseOrderModal fetchLines — ETP-4735 pricing', () => {
  ...
  it('does not hardcode _unitPrice or _lineNetAmount to 0 anywhere in fetchLines', () => {
    // Isolate the fetchLines function body so this doesn't false-positive on
