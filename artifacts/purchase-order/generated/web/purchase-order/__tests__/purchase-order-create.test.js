/**
 * Purchase Order — structural source-reading tests.
 *
 * Replaces e2e/tests/flows/purchase-order-create.spec.js which only checked
 * visibility of static elements (columns, fields, partner address dependency).
 */
...
describe('Purchase Order — HeaderTable', () => {
  ...
  it('declares filters including documentStatus and businessPartner', () => {
    assert.match(headerTable, /filters.*businessPartner/);
    assert.match(headerTable, /filters.*documentStatus/);
  });
});

describe('Purchase Order — HeaderForm', () => {
...
describe('Purchase Order — HeaderPage', () => {
