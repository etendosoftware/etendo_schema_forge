 ...
 * missed this panel and its payment-in twin). Delegates to the canonical formatAmount/formatCurrency
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', padding: '8px 20px 12px', gap: 44 }}>
      <div style={{ width: 148, flexShrink: 0 }}>
        <div style={{ font: '600 14px/20px Inter', color: 'hsl(var(--foreground))' }}>{ui('paymentOutDataTitle')}</div>
        <div style={{ font: '400 12px/16px Inter', color: 'hsl(var(--foreground))', marginTop: 4 }}>{ui('paymentOutDataSub')}</div>
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'flex', flexDirection: 'row', gap: 20 }}>
          <FieldItem label={ui('docNo')}>{data?.documentNo}</FieldItem>
          <FieldItem label={ui('Vendor')}>{data?.['businessPartner$_identifier']}</FieldItem>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: 20 }}>
          <FieldItem label={ui('payFrom')}>{data?.['account$_identifier']}</FieldItem>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', padding: '8px 20px 12px', gap: 16 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ font: '600 14px/20px Inter', color: 'hsl(var(--foreground))' }}>{ui('paymentOutLinesTitle')}</div>
        <div style={{ font: '400 12px/16px Inter', color: 'hsl(var(--foreground))' }}>{ui('paymentOutLinesSub')}</div>
      </div>

      {lines === null ? (
        <div style={{ font: '400 13px/18px Inter', color: 'hsl(var(--muted-foreground))' }}>{ui('loading')}</div>
      ) : noLines ? (
        <div style={{ font: '400 13px/18px Inter', color: 'hsl(var(--muted-foreground))' }}>{ui('paymentOutNoLines')}</div>
      ) : (
