...
function GeneralFields({ data }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12,1fr)', gap: '16px 18px' }}>
      ...
      <KV label="Cliente" span={6}>{data?.['businessPartner$_identifier']}</KV>
      ...
      <KV label="Depositar en" span={5}>{data?.['account$_identifier']}</KV>
...
function AppliedLinesTable({ data, token, apiBaseUrl }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${base}/payment-in/finPaymentScheduleDetail?parentId=${data.id}&_startRow=0&_endRow=100`, { headers });
  ...
  if (!data?.id) {
    return <div style={{ padding: '12px 0', color: '#828FA3', font: '400 13px/18px Inter' }}>Sin líneas de cobro.</div>;
  }
  if (lines === null) {
...
export default function FinPaymentForm({ data, token, apiBaseUrl, section, ...props }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, background: '#FAFAFB', minHeight: '100%' }}>
      {isDraft && (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9, padding: '10px 14px', background: '#FFF9EB', borderRadius: 8, border: '1px solid #F2E2BC' }}>
          ...
          <span style={{ font: '500 12px/16px Inter', color: '#8A6E25' }}>Borrador — sin efecto sobre la cuenta financiera. Al confirmar, el importe se deposita de inmediato y se reduce el saldo de la factura.</span>
        </div>
      )}
      <Card title="Datos del cobro">
        <GeneralFields data={data} />
      </Card>
      <Card title="Líneas del cobro" desc="Facturas de venta saldadas con este cobro.">
        <AppliedLinesTable data={data} token={token} apiBaseUrl={apiBaseUrl} />
      </Card>
    </div>
  );
}
