...
export default function GoodsShipmentActions({ data, recordId, token, apiBaseUrl, api, onRefresh }) {
  ...
  // Fetches this shipment's own lines (product + salesOrderLine) and its pending-quantity map —
  // both needed for the quote, not just for a total. Skipped entirely when a linked order
  // exists (see hasLinkedOrder above): the quote would be misleading there, not just imprecise.
  useEffect(() => {
    ...
    (async () => {
      const [lineRes, pendingRes] = await Promise.all([
        apiFetch(`${base}/goods-shipment/goodsShipmentLine?parentId=${recordId}&_startRow=0&_endRow=200`, { baseUrl: '', token })
          .catch(() => null),
        apiFetch(`${base}/goods-shipment/goodsShipment/${recordId}/action/pendingInvoiceLines`, { baseUrl: '', token })
          .catch(() => null),
      ]);
      if (cancelled) return;
