...
const src = readFileSync(join(__dirname, '..', 'ImportFromSalesInvoiceModal.jsx'), 'utf8');

describe('ImportFromSalesInvoiceModal', () => {
  it('exports a default function component', () => {
    assert.match(src, /export default function ImportFromSalesInvoiceModal/);
  });
  ...
  describe('ETP-4028 — currency-aware fetchDocuments', () => {
    ...
    it('computes status/BP candidates before applying the currency filter', () => {
      assert.match(
        src,
    ...
    it('builds the ordersByInvoiceId map only from the currency-filtered documents', () => {
      assert.match(
        src,
        /documents\.forEach\(doc => \{\s*\n\s*if \(doc\.salesOrder\) ordersByInvoiceId\[doc\.id\] = doc\.salesOrder;\s*\n\s*\}\);/,
      );
    });

    it('returns documents, sharedContext, and excludedByCurrency from fetchDocuments', () => {
      assert.match(src, /return \{ documents, sharedContext: \{ ordersByInvoiceId, draftInfo \}, excludedByCurrency \};/);
    });
  });

  it('passes noSalesInvoicesMatchShipmentCurrency as the noCurrencyMatchMessageKey', () => {
    assert.match(src, /noCurrencyMatchMessageKey="noSalesInvoicesMatchShipmentCurrency"/);
  });
});
