...
**Update (2026-08-30):** with the epic tier retired, the release cadence is now two steps, not three:
`feature → develop` (the merge block, this is my job), then, once that lands, `develop → main` (→ production,
promotion PR, none of my business).

What this means for me:
- If a block was pre-flighted **before** a prior block already landed on `develop`, its verification may be
  **stale** if `develop`'s head moved underneath it (e.g. another block merged first). Re-run the pre-flight
