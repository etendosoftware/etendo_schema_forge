...
| merge-block-helper.md | Blockie | MERGE BLOCK PRE-FLIGHT — given a dev task (ETP-XXXX), checks its `feature/ETP-XXXX` branch + PR across the 3 repos, verifies CI/review/mergeability/target/code-owner gate, reports a traffic-light readiness table. Merges (plain local `git merge` into the epic) ONLY the branches the human explicitly authorizes; never touches the PRs (they auto-close on push), never pushes | Diagnostic |
...
| "Merge the ones I told you into the epic" | **Blockie** | Human-authorized local `git merge` into the current epic |
