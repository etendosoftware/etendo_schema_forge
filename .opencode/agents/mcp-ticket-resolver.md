...
**MCP-facing metadata (read before coding if the ticket touches it):** anything the agent *sees* about a spec/entity/field — spec `agentPrompt` & `type`, entity `name`/`methods`/`Java_Qualifier`, or a field's `businessCritical`, per-field `agentPrompt`, `visibility`, or `defaultValue` — is either driven by `decisions.json` (persisted to `ETGO_SF_{SPEC,ENTITY,FIELD}`) or read live from AD. The NEO/MCP Java only reads and surfaces it; it never decides the decisions-driven parts. So a ticket asking to *change* one of those is **upstream-config**, NOT a Java fix. `docs/agentic-validation/mcp-field-flags-pipeline.md` is the canonical map: per-level (spec/entity/field) it lists every agent-visible key, which tool surfaces it (with `file:line` anchors), its `ETGO_SF_*` column, and its `decisions.json` source — plus the `decisions.json` → `make regen ONLY=<window> PUSH_TO_NEO=1` → `./gradlew export.database` fix recipe.
