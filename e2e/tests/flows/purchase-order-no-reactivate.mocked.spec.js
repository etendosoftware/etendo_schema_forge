 ...
 *       ETP-5315 `PurchaseOrderReactivateBulkAction` button offering `RE`,
 *       separate from the pre-existing CO-only button.
 *
 * ETP-5302 renamed the CO-only button to "Procesar" and ETP-5315 gave the
 * reactivate one its own `reactivateBulk` key ("Reactivar"), so the two are
 * told apart by name here rather than by position.
 */
...
test.describe('Purchase Order list — bulk-select Reactivate (ETP-5315)', () => {
  ...
  test('selecting only draft rows does not offer RE (only CO)', async ({ page }) => {
    ...
    // Only ONE button renders here: PurchaseOrderReactivateBulkAction's
    ...
    // ETP-5302 renamed that button from "Confirmar" to "Procesar": it opens the
    // action picker rather than confirming anything by itself, and "Confirmar" is
    // now the name of the CO option INSIDE the dialog.
    const processButtons = page.getByRole('button', { name: /^(procesar|process)$/i });
    await expect(processButtons.first()).toBeVisible({ timeout: 5_000 });
