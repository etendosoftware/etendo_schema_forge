...
test.describe('Purchase Order — Full flow with receipt and invoice (integration)', () => {
  ...
  /**
   * ETP-4772 regression guard: a user-selected Warehouse used to be silently
   * reverted to the callout's default value when the PO was saved (a stale
   * callout response overwrote the field the user had just changed). Fixed
   * via a per-field "generation" guard in `useCallout.js` / `DetailView.jsx`
   * (`fieldGenerationRef` + `dispatchSnapshot`) that discards outdated
   * callout responses. This test exercises the full create → change → save
   * → reload cycle so a future refactor that forgets to wire that guard into
   * the detail-view context is caught here, not just by the unit tests that
   * cover the guard function in isolation.
   *
   * Flow:
   *   1. Create a new PO, select the vendor BP.
   *   2. Wait for the Warehouse field to resolve its callout default
   *      (establishing one manually if the environment doesn't auto-resolve
   *      one, so the "change" step always has a real baseline to diverge from).
   *   3. Re-open the Warehouse selector and pick a DIFFERENT option than the
   *      default.
   *   4. Save the PO as a draft.
   *   5. Reload the page — forces a real GET against the backend instead of
   *      trusting in-memory client state — and verify the Warehouse still
   *      shows the manually chosen value, not the original default.
   */
  test('creating a new PO keeps the manually selected Warehouse after saving (ETP-4772 regression guard)', async ({ page }) => {
    const user = onboardingCreds?.email || process.env.E2E_USER;
    const password = onboardingCreds?.password || process.env.E2E_PASSWORD;

    // ═══════════════════════════════════════════════════════════════════════
    // STEP 1: Login + ensure the vendor is set up (independent test, no
    // shared state with the flow above)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 2: Create a new PO and select the vendor BP
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 3: Wait for the Warehouse callout default to resolve. If this
    // environment doesn't auto-resolve one, establish a baseline manually by
    // picking the first available option — the regression is specifically
    // about a value getting REVERTED, so we need a real default to diverge
    // from either way.
    // ═══════════════════════════════════════════════════════════════════════
    ...
    let warehouseChip = page.getByTestId('field-warehouse-chip');
    if (!await warehouseChip.isVisible({ timeout: 8_000 }).catch(() => false)) {
      const warehouseInput = page.getByTestId('field-warehouse');
      await expect(warehouseInput,
        'Warehouse field should be visible and selectable',
      ).toBeVisible({ timeout: 10_000 });
      await warehouseInput.click();
      const firstOption = page.locator('[data-testid^="option-warehouse-"]').first();
      await expect(firstOption).toBeVisible({ timeout: 10_000 });
      await firstOption.click();
      await slow(page);
      warehouseChip = page.getByTestId('field-warehouse-chip');
      await expect(warehouseChip,
        'Warehouse chip should render once a value is selected',
      ).toBeVisible({ timeout: 5_000 });
    }

    const defaultWarehouseLabel = (await warehouseChip.textContent())?.trim();
    expect(defaultWarehouseLabel, 'Default Warehouse label should be readable').toBeTruthy();

    // ═══════════════════════════════════════════════════════════════════════
    // STEP 4: Re-open the selector and pick a DIFFERENT Warehouse
    // ═══════════════════════════════════════════════════════════════════════

    await warehouseChip.click(); // flips the chip back to an editable combobox

    // NOTE on timing (ETP-4772 flake investigation): the Warehouse field renders through
    // CreatableSearchSelect in `serverSearch` mode (see EntityForm.jsx's
    // renderSearchSelectField — every plain FK `type:'selector'` field defaults to it since
    // ETP-4600), so re-opening the chip re-focuses the input and re-triggers a fresh
    // `/selectors/warehouse` fetch. But no single request URL/timing is reliable enough to
    // hook with `waitForResponse` here (confirmed empirically: a request wasn't always
    // observed on the exact re-open, yet the options still populated a bit later from a
    // different in-flight fetch). The REAL bug in the previous version of this test was
    // `otherWarehouseOption.isVisible({ timeout })`: `Locator.isVisible()` does NOT poll —
    // it evaluates the current DOM state once and returns immediately, so raising its
    // `timeout` from 5s to 15s changed nothing (confirmed empirically: total test duration
    // stayed ~11s and the option count was still 0 right after the check, meaning it never
    // actually waited). `Locator.waitFor({ state: 'visible' })` is the primitive that
    // actually polls up to `timeout`, so it's used below instead.
    const otherWarehouseOption = page.locator('[data-testid^="option-warehouse-"]')
      .filter({ hasNotText: defaultWarehouseLabel }).first();

    const otherWarehouseVisible = await otherWarehouseOption
      .waitFor({ state: 'visible', timeout: 15_000 })
      .then(() => true)
      .catch(() => false);

    if (!otherWarehouseVisible) {
      test.skip(true, 'Environment only exposes one Warehouse option — cannot exercise a real change');
    }

    const chosenWarehouseLabel = (await otherWarehouseOption.textContent())?.trim();
    await otherWarehouseOption.click();
    await slow(page);

    const chipAfterChange = page.getByTestId('field-warehouse-chip');
    await expect(chipAfterChange,
      'Chip should reflect the newly picked Warehouse before saving',
    ).toHaveText(chosenWarehouseLabel, { timeout: 5_000 });

    // ═══════════════════════════════════════════════════════════════════════
    // STEP 5: Save the PO as a draft
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 6: Reload — forces a real GET against the backend, not just the
    // in-memory client state — and verify the persisted Warehouse is the one
    // the user chose, NOT the original callout default.
    // ═══════════════════════════════════════════════════════════════════════

    await safeReload(page);
    await waitForDetailReady(page);

    const persistedChip = page.getByTestId('field-warehouse-chip');
    await expect(persistedChip,
      '[ETP-4772 regression] Warehouse chip should be visible after reload',
    ).toBeVisible({ timeout: 10_000 });
    const persistedLabel = (await persistedChip.textContent())?.trim();

    expect(
      persistedLabel,
      `[ETP-4772 regression] Warehouse should persist the chosen value "${chosenWarehouseLabel}", ` +
      `not revert to the original default "${defaultWarehouseLabel}". Got: "${persistedLabel}"`,
    ).toBe(chosenWarehouseLabel);
    expect(
      persistedLabel,
      '[ETP-4772 regression] Warehouse must NOT revert to the original callout default',
    ).not.toBe(defaultWarehouseLabel);
