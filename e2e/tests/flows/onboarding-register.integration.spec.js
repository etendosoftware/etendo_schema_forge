...
test.describe('Onboarding — Register new user (integration)', () => {
  ...
  test('registers a new user, selects Autónomo, and verifies greeting', async ({ page }) => {
    ...
    // client/org bootstrap. This is a hard blocker for the "onboarding-setup" Playwright
    // project, which every other integration spec depends on for login credentials — without
    // this fixme, the entire integration suite fails to even start. Tracked for a fix in
    // com.etendoerp.go; re-enable once resolved.
