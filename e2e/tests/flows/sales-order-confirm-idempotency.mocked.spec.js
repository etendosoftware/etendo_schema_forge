...
const NO_DOC_ORDER_ID = 'idem-mock-order-nodoc-001';
...
const NO_DOC_HEADER = {
  ...
  summedLineAmount: 80,
  totalLines: 80,
  'businessPartner$_identifier': 'Test BP No Doc',
  'currency$_identifier': 'EUR',
};

test.describe('Sales Order — Confirm without related documents (ETP-5063)', () => {
  test.beforeEach(async ({ page }) => {
    await login(page);
  });

  test('confirming with neither shipment nor invoice checked shows a toast (not the result modal) and refreshes the header', async ({ page }) => {
    let headerGetCount = 0;
    page.on('request', (req) => {
      if (req.method() === 'GET' && req.url().includes(`/sales-order/header/${NO_DOC_ORDER_ID}`)) {
        headerGetCount += 1;
      }
    });

    await installHeaderConfirmMock(page, { spec: 'sales-order', recordId: NO_DOC_ORDER_ID, record: NO_DOC_HEADER });
    ...
    await page.route(
      `**/sws/neo/sales-order/header/${NO_DOC_ORDER_ID}/action/documentAction`,
      async (route) => {
    ...
    await page.goto(`/sales-order/${NO_DOC_ORDER_ID}`);
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByTestId('detail-view')).toBeVisible({ timeout: 8_000 });
    ...
    // Open the confirm modal WITHOUT ticking either checkbox. The primary
    // button reads ui('soConfirmActionOnly') = "Confirmar pedido" (no
    // stable data-testid on this modal's confirm button, unlike PO's).
    const confirmBtn = page.getByRole('button', { name: 'Confirmar pedido', exact: true });
    for (let attempt = 0; attempt < 5; attempt++) {
      await page.evaluate(() => {
        window.dispatchEvent(new CustomEvent('sales-order:open-confirm-modal'));
      });
    ...
    // soConfirmedTitle text (es_ES: "Pedido confirmado").
    await expect(successToast).toContainText('Pedido confirmado');
